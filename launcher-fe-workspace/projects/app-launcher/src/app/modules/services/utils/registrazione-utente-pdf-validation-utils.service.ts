...
export class RegistrazioneUtentePdfValidationUtilsService extends AbstractValidationUtilsService {
    ...
    protected elaborateValidOne(field: SdkFormBuilderField, valids: Array<boolean>, messagesPanel: HTMLElement, messagesConfig: Array<SdkMessagePanelConfig>): Array<boolean> {
        if (isObject(field)) {
            if (field.code === 'upload-documento' || field.code === 'trattamento-dati-personali') {
                valids.push(true);
            } else {
