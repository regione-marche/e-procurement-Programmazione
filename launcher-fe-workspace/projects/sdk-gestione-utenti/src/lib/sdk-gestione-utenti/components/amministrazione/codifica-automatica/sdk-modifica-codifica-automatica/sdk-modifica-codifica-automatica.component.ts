...
@Component({
  templateUrl: './sdk-modifica-codifica-automatica.component.html'
})
export class SdkModificaCodificaAutomaticaComponent extends SdkBusinessAbstractWidget<void> implements OnInit, OnDestroy {

  // #region Variables

  @HostBinding('class') classNames = `sdk-modifica-codifica-automatica-section`;
  ...
  private nomEnt: string;
  private nomCam: string;
  private tipCam: number;
  private dettaglio: GConfCodDTO;
  ...
  public onButtonClick(button: SdkButtonGroupOutput): void {
    if (button != null && isEmpty(button.provider) === false) {
      ...
      if (button.code === 'back-to-dettaglio-codifica-automatica' && isEqual(this.defaultFormBuilderConfig, this.formBuilderConfig) === false) {
    ...
    if (this.config != null) {
      this.markForCheck(() => {
        ...
        let form: IDictionary<any> = {
          ...this.dettaglio,
          codificaAutomaticaAttiva: this.dettaglio.codCal != null && this.dettaglio.contat != null ? '1' : '2'
        };
  ...
  private loadQueryParams(): void {
    ...
    this.nomEnt = paramsMap.get('nomEnt');
    this.nomCam = paramsMap.get('nomCam');
    this.tipCam = +paramsMap.get('tipCam');
  }

  private loadDettaglio = (): Observable<ResponseDTO<GConfCodDTO>> => {
    return this.gestioneCodificaAutomaticaService.getDettaglioCodificaAutomatica(this.nomEnt, this.nomCam, this.tipCam)
      .pipe(
  ...
  private elaborateDettaglio = (result: ResponseDTO<GConfCodDTO>) => {
