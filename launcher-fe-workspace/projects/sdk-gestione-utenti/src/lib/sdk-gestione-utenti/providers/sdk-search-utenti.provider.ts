...
export interface SdkGestioneUtentiSearchProviderArgs extends IDictionary<any> {
...
@Injectable()
export class SdkGestioneUtentiSearchProvider extends SdkBaseService implements SdkProvider {

    constructor(inj: Injector) {
        super(inj);
    }

    public run(args: SdkGestioneUtentiSearchProviderArgs): Observable<IDictionary<any>> {
        this.logger.debug('SdkGestioneUtentiSearchProvider', args);
        ...
        if (args.buttonCode === 'back-button') {
            ...
            let form: RicercaUtentiFormDTOInternal = this.populateRequest(formBuilderConfig);
            this.store.dispatch(new SdkStoreAction(SdkGestioneUtentiConstants.SEARCH_FORM_UTENTI_DISPATCHER, form));
            this.routerService.navigateToPage('lista-utenti-page');
        }
        return new Observable((ob: Observer<IDictionary<any>>) => {
            ob.next(args);
            ob.complete();
    ...
    private populateRequest(formBuilderConfig: SdkFormBuilderConfiguration): RicercaUtentiFormDTOInternal {

        let request: RicercaUtentiFormDTOInternal = this.getDefaultForm();
    ...
    private getDefaultForm(): RicercaUtentiFormDTOInternal {
        let item: RicercaUtentiFormDTOInternal = {};
        return item;
    }
