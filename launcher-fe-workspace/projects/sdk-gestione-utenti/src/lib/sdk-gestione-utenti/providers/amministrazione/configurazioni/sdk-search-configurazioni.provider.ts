...
import { RicercaConfigurazioniFormDTO } from '../../../model/amministrazione.model';
import { SdkGestioneUtentiConstants } from '../../../sdk-gestione-utenti.constants';


export interface SdkGestioneConfigurazioniSearchProviderArgs extends IDictionary<any> {
...
@Injectable()
export class SdkGestioneConfigurazioniSearchProvider extends SdkBaseService implements SdkProvider {

    constructor(inj: Injector) {
        super(inj);
    }

    public run(args: SdkGestioneConfigurazioniSearchProviderArgs): Observable<IDictionary<any>> {
        this.logger.debug('SdkGestioneConfigurazioniSearchProviderArgs >>>', args);
        ...
        if (args.buttonCode === 'back-button') {
            ...
            let form: RicercaConfigurazioniFormDTO = this.populateRequest(formBuilderConfig);
            this.store.dispatch(new SdkStoreAction(SdkGestioneUtentiConstants.SEARCH_FORM_GESTIONE_CONFIGURAZIONI_DISPATCHER, form));
            this.routerService.navigateToPage('lista-configurazioni-page');
        }
        return new Observable((ob: Observer<IDictionary<any>>) => {
            ob.next(args);
            ob.complete();
    ...
    private populateRequest(formBuilderConfig: SdkFormBuilderConfiguration): RicercaConfigurazioniFormDTO {

        let request: RicercaConfigurazioniFormDTO = this.getDefaultForm();
    ...
    private getDefaultForm(): RicercaConfigurazioniFormDTO {
        let item: RicercaConfigurazioniFormDTO = {};
        return item;
    }
