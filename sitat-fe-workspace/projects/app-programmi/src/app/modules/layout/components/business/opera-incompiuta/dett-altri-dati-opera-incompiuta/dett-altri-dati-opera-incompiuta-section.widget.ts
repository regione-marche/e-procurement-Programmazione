...
import { cloneDeep, each, get, has, isEmpty, isObject, isString, remove, toString } from 'lodash-es';
...
@Component({
    templateUrl: `dett-altri-dati-opera-incompiuta-section.widget.html`,
    encapsulation: ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.OnPush
})
export class DettaglioAltriDatiOperaIncompiutaSectionWidget extends SdkBusinessAbstractWidget<void> implements OnInit, OnDestroy {

    // #region Variables

    @HostBinding('class') classNames = `dett-altri-dati-opera-incompiuta-section`;
    ...
    private loadForm(): void {
        ...
        let customPopulateFunction: CustomParamsFunction = (field: SdkFormBuilderField, restObject: any) => {
            ...
            if (field.code === 'provincia') {
                if (has(this.operaIncompiuta, 'codIstat') && this.operaIncompiuta.codIstat != null) {
                    let codIstatProvincia = '0' + this.operaIncompiuta.codIstat.substring(3, 6);
                    field.data = this.provinceMap[codIstatProvincia];
                }
                mapping = false;
            } else if (field.code === 'comune' && this.operaIncompiuta.codIstat != null) {
                if (has(this.operaIncompiuta, 'codIstat')) {
                    field.data = this.comuniMap[this.operaIncompiuta.codIstat]
                }
                mapping = false;
            }
