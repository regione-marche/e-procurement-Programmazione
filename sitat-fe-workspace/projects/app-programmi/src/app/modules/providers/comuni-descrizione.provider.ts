...
export class ComuniDescrizioneProvider extends SdkBaseService implements SdkProvider {
    ...
    public run(args: any): Function {
        return (data?: string): Observable<Array<SdkAutocompleteItem>> => {
            let func = this.getListaComuniFactory(data);
            return this.requestHelper.begin(func, undefined, false);
        }
    }

    private getListaComuniFactory(data?: string): () => Observable<any> {
        return () => {
