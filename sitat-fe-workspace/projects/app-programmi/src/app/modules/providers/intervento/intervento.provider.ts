...
export class InterventoProvider extends SdkBaseService implements SdkProvider {
        ...
        if (args.buttonCode === 'back-to-dett-prog') {
            ...
            if (!isEqual(defaultFormBuilderConfig, formBuilderConfig)) {
                ...
                if (valid === true) {
                    ...
                    let inserisciInterventoFactory = this.inserisciInterventoFactory(request);
                    return this.requestHelper.begin(inserisciInterventoFactory, messagesPanel).pipe(map((result: any): IDictionary<any> => {
                        let idIntervento: number = result.data;
    ...
    private inserisciInterventoFactory(insertForm: InterventoInsertForm) {
        return () => {
            return this.programmiService.createIntervento(insertForm)
        }
    }
