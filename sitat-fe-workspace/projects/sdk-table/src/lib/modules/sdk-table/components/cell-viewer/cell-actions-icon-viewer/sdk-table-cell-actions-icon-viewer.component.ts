...
@Component({
  templateUrl: 'sdk-table-cell-actions-icon-viewer.component.html',
  styleUrls: ['sdk-table-cell-actions-icon-viewer.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class SdkTableCellActionsIconViewerComponent extends SdkAbstractCellViewer implements SdkTableCellViewer {

  @HostBinding('class') styles = 'sdk-table-cell-actions-icon-viewer';

  public actions: Array<SdkTableActionConfig> = [];

  public disablePointer: boolean = false;
  ...
  public clickHandler(_event: MouseEvent, act: SdkTableActionConfig): void {
    try {
      if (isObject(act) && act.disabled !== true && isFunction(act.performer)) {
  ...
  protected onColumnConfigChange(): void {
    try {
      ...
      // Logica per disabilitare il cursore solo per le azioni disabilitate e quindi intese come icone.
      this.disablePointer = isArray(actions) && actions.every(action => action.disablePointer);
    } catch (e) { }
  }
