...
export class ImpreseAggiudicatarieAggiSubaAutocompleteProvider extends SdkBaseService implements SdkProvider {
    ...
    private getListaOpzioniImpreseAggiudicatarieFactory(stazioneAppaltante: StazioneAppaltanteInfo, codGara: string, codLotto: string, data?: string): () => Observable<any> {
        return () => {
            return this.archivioImpreseService.getImpreseOptionsAggiSuba(environment.GESTIONE_CONTRATTI_BASE_URL, stazioneAppaltante, codGara, codLotto, data)
                .pipe(
