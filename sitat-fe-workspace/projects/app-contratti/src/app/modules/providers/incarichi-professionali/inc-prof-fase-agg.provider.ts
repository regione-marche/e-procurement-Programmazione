...
export interface IncarichiProfessionaliFaseAggiudicazioneProviderArgs extends IDictionary<any> {
...
export class IncarichiProfessionaliFaseAggiudicazioneProvider extends SdkBaseService implements SdkProvider {

    constructor(inj: Injector) {
        super(inj);
    }

    public run(args: IncarichiProfessionaliFaseAggiudicazioneProviderArgs): Observable<IDictionary<any>> {
        this.logger.debug('IncarichiProfessionaliFaseAggiudicazioneProviderArgs >>>', args);
    ...
    private dettaglioIncarichiProfessionali(args: IncarichiProfessionaliFaseAggiudicazioneProviderArgs): Observable<IDictionary<any>> {
        ...
        this.routerService.navigateToPage('dettaglio-inc-prof-fase-agg-page', params);
    ...
    private goToUpdateIncarichiProfessionali(args: IncarichiProfessionaliFaseAggiudicazioneProviderArgs): Observable<IDictionary<any>> {
        let params: IDictionary<any> = {
            codGara: args.codGara,
            codLotto: args.codLotto,
            codiceFase: args.codiceFase,
        ...
        this.routerService.navigateToPage('modifica-inc-prof-fase-agg-page', params);
        return new Observable((ob: Observer<IDictionary<any>>) => {
            ob.next(undefined);
            ob.complete();
        });
    ...
    private saveIncarichi(args: IncarichiProfessionaliFaseAggiudicazioneProviderArgs): Observable<any> {
        ...
        if (!isEqual(defaultFormBuilderConfig, formBuilderConfig)) {
            ...
            if (valid === true) {
                ...
                each(request.incarichi, (one: IncaricoProfessionaleInsertForm) => {
                    one.sezione = one.idRuolo >= 5 && one.idRuolo <= 18 ? 'RA' : 'PA';
                });
