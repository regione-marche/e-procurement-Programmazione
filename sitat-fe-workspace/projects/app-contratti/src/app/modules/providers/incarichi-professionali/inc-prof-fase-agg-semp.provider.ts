...
export interface IncarichiProfessionaliFaseAggiudicazioneSempProviderArgs extends IDictionary<any> {
...
export class IncarichiProfessionaliFaseAggiudicazioneSempProvider extends SdkBaseService implements SdkProvider {

    constructor(inj: Injector) {
        super(inj);
    }

    public run(args: IncarichiProfessionaliFaseAggiudicazioneSempProviderArgs): Observable<IDictionary<any>> {
        this.logger.debug('IncarichiProfessionaliFaseAggiudicazioneSempProviderArgs >>>', args);
    ...
    private dettaglioIncarichiProfessionali(args: IncarichiProfessionaliFaseAggiudicazioneSempProviderArgs): Observable<IDictionary<any>> {
        ...
        this.routerService.navigateToPage('dettaglio-inc-prof-fase-agg-semp-page', params);
    ...
    private goToUpdateIncarichiProfessionali(args: IncarichiProfessionaliFaseAggiudicazioneSempProviderArgs): Observable<IDictionary<any>> {
        let params: IDictionary<any> = {
            codGara: args.codGara,
            codLotto: args.codLotto,
            codiceFase: args.codiceFase,
        ...
        this.routerService.navigateToPage('modifica-inc-prof-fase-agg-semp-page', params);
        return new Observable((ob: Observer<IDictionary<any>>) => {
            ob.next(undefined);
            ob.complete();
        });
    ...
    private saveIncarichi(args: IncarichiProfessionaliFaseAggiudicazioneSempProviderArgs): Observable<any> {
        ...
        const lotto: LottoEntry = args.lotto;
        ...
        if (!isEqual(defaultFormBuilderConfig, formBuilderConfig)) {
            ...
            if (valid === true) {
                ...
                each(request.incarichi, (one: IncaricoProfessionaleInsertForm) => {
                    one.sezione = lotto.artE1 === '1' ? 'RE' : 'RS';
                });

                let factory: Function = this.updateIncarichiFactory(request);
