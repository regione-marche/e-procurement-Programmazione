...
export class CredenzialiSimogModalProvider extends SdkBaseService implements SdkProvider {
    ...
    public run(args: IDictionary<any>): Observable<IDictionary<any>> {
        this.logger.debug('CredenzialiSimogModalProvider >>>', args);

        let data: IDictionary<any> = get(args, 'data');

        if (isObject(data)) {
            if (get(data, 'code') === 'confirm-data') {
    ...
    private get genericValidationUtilsService(): GenericValidationUtilsService { return this.injectable(GenericValidationUtilsService) }
