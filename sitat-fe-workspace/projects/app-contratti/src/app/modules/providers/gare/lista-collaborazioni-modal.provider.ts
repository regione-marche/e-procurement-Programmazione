...
export class ListaCollaborazioniModalProvider extends SdkBaseService implements SdkProvider {
    ...
    public run(args: IDictionary<any>): Observable<IDictionary<any>> {
        this.logger.debug('ListaCollaborazioniModalProvider >>>', args);

        let data: IDictionary<any> = get(args, 'data');

        if (isObject(data)) {
            if (get(data, 'code') === 'choose-collaborazione') {
