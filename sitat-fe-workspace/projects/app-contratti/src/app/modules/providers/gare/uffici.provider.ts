...
export class UfficiProvider extends SdkBaseService implements SdkProvider {
    ...
    private getListaUfficiFactory(stazioneAppaltante: StazioneAppaltanteInfo, data?: string): () => Observable<any> {
        return () => {
            return this.tabellatiService.listaOpzioniUfficiAvvisi(environment.GESTIONE_TABELLATI_BASE_URL, stazioneAppaltante.codice, data)
                .pipe(
