...
import { FaseAdesioneAccordoQuadroInsertForm } from '../../models/fasi/fase-adesione-accordo-quadro.model';
...
export class FaseAdesioneAccordoQuadroProvider extends SdkBaseService implements SdkProvider {
    ...
    public run(args: IDictionary<any>): Observable<IDictionary<any>> {
        this.logger.debug('FaseAdesioneAccordoQuadroProvider >>>', args);
        ...
        if (args.buttonCode === 'back-to-dettaglio-fase') {
            ...
            if (!isEqual(defaultFormBuilderConfig, formBuilderConfig)) {
                ...
                if (valid === true) {
                    ...
                    let request: FaseAdesioneAccordoQuadroInsertForm = this.populateRequest(formBuilderConfig, codGara, codLotto);
                    ...
                    return this.requestHelper.begin(faseFactory, messagesPanel).pipe(
                        map((result: any) => {
                            ...
                            this.routerService.navigateToPage('dettaglio-fase-adesione-accordo-quadro-page', params);
                            return {
                                ...result,
    ...
    private modificaFase(codGara: string, codLotto: string, codiceFase: string, numeroProgressivo: string): Observable<IDictionary<any>> {
        ...
        this.routerService.navigateToPage('modifica-fase-adesione-accordo-quadro-page', params);
        return new Observable((ob: Observer<IDictionary<any>>) => {
            ob.next(undefined);
            ob.complete();
        });
    ...
    private insertFaseFactory(insertForm: FaseAdesioneAccordoQuadroInsertForm) {
        return () => {
            return this.faseAdesioneAccordoQuadroService.insertFase(insertForm);
        }
    }

    private updateFaseFactory(insertForm: FaseAdesioneAccordoQuadroInsertForm) {
        return () => {
            return this.faseAdesioneAccordoQuadroService.updateFase(insertForm);
        }
    }

    private getDefaultFaseForm(codGara: string, codLotto: string): FaseAdesioneAccordoQuadroInsertForm {
        const request: FaseAdesioneAccordoQuadroInsertForm = {
            codGara: +codGara,
            codLotto: +codLotto
        };
        return request;
    ...
    private elaborateOne(field: SdkFormBuilderField, request: FaseAdesioneAccordoQuadroInsertForm, dynamicMappingOutput?: string): FaseAdesioneAccordoQuadroInsertForm {
        if (isObject(field)) {
    ...
    private elaborateSection(field: SdkFormBuilderField, request: FaseAdesioneAccordoQuadroInsertForm): FaseAdesioneAccordoQuadroInsertForm {
    ...
    private elaborateGroup(field: SdkFormBuilderField, request: FaseAdesioneAccordoQuadroInsertForm): FaseAdesioneAccordoQuadroInsertForm {
    ...
    private populateRequest(formBuilderConfig: SdkFormBuilderConfiguration, codGara: string, codLotto: string): FaseAdesioneAccordoQuadroInsertForm {
        let request: FaseAdesioneAccordoQuadroInsertForm = this.getDefaultFaseForm(codGara, codLotto);
