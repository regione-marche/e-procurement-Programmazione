...
import { FaseInadempienzeSicurezzaInsertForm } from '../../models/fasi/fase-inadempienze-sicurezza.model';
...
export class FaseInadempienzeSicurezzaProvider extends SdkBaseService implements SdkProvider {
    ...
    public run(args: IDictionary<any>): Observable<IDictionary<any>> {
        this.logger.debug('FaseInadempienzeSicurezzaProvider >>>', args);
        ...
        if (args.buttonCode === 'back-to-dettaglio-fase') {
            ...
            if (valid === true) {
                ...
                const request: FaseInadempienzeSicurezzaInsertForm = this.populateRequest(formBuilderConfig, codGara, codLotto);
                ...
                return this.requestHelper.begin(faseFactory, messagesPanel).pipe(
                    map((result: any) => {
                        ...
                        this.routerService.navigateToPage('dettaglio-fase-inadempienze-sicurezza-page', params);
                        return {
                            ...result,
    ...
    private modificaFase(codGara: string, codLotto: string, codiceFase: string, numeroProgressivo: string): Observable<IDictionary<any>> {
        ...
        this.routerService.navigateToPage('modifica-fase-inadempienze-sicurezza-page', params);
        return new Observable((ob: Observer<IDictionary<any>>) => {
            ob.next(undefined);
            ob.complete();
        });
    ...
    private insertFaseFactory(insertForm: FaseInadempienzeSicurezzaInsertForm) {
        return () => {
            return this.faseInadempienzeSicurezzaService.insertFase(insertForm);
        }
    }

    private updateFaseFactory(insertForm: FaseInadempienzeSicurezzaInsertForm) {
        return () => {
            return this.faseInadempienzeSicurezzaService.updateFase(insertForm);
        }
    }

    private getDefaultFaseForm(codGara: string, codLotto: string): FaseInadempienzeSicurezzaInsertForm {
        const request: FaseInadempienzeSicurezzaInsertForm = {
            codGara: +codGara,
            codLotto: +codLotto
        };
        return request;
    ...
    private elaborateOne(field: SdkFormBuilderField, request: FaseInadempienzeSicurezzaInsertForm, dynamicMappingOutput?: string): FaseInadempienzeSicurezzaInsertForm {
        if (isObject(field)) {
    ...
    private elaborateSection(field: SdkFormBuilderField, request: FaseInadempienzeSicurezzaInsertForm): FaseInadempienzeSicurezzaInsertForm {
    ...
    private elaborateGroup(field: SdkFormBuilderField, request: FaseInadempienzeSicurezzaInsertForm): FaseInadempienzeSicurezzaInsertForm {
    ...
    private populateRequest(formBuilderConfig: SdkFormBuilderConfiguration, codGara: string, codLotto: string): FaseInadempienzeSicurezzaInsertForm {
        let request: FaseInadempienzeSicurezzaInsertForm = this.getDefaultFaseForm(codGara, codLotto);
