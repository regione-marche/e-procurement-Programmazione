...
import { FaseVariazioneAggiudicatariInsertForm } from '../../models/fasi/fase-variazione-aggiudicatari.model';
import { FaseVariazioneAggiudicatariService } from '../../services/fasi/fase-variazione-aggiudicatari.service';


@Injectable({ providedIn: 'root' })
export class FaseVariazioneAggiudicatariProvider extends SdkBaseService implements SdkProvider {
    ...
    public run(args: IDictionary<any>): Observable<IDictionary<any>> {
        this.logger.debug('FaseCollaudoProvider >>>', args);
        ...
        if (args.buttonCode === 'back-to-dettaglio-fase') {
            ...
            if (valid === true) {
                ...
                const request: FaseVariazioneAggiudicatariInsertForm = this.populateRequest(formBuilderConfig, codGara, codLotto);

                let faseFactory: Function;

                if (isUpdatePhase === true) {
                    request.numVaraggi = +numeroProgressivo;
                ...
                return this.requestHelper.begin(faseFactory, messagesPanel).pipe(
                    map((result: any) => {
                        ...
                        this.routerService.navigateToPage('dettaglio-fase-variazione-aggiudicatari-page', params);
                        return {
                            ...result,
    ...
    private modificaFase(codGara: string, codLotto: string, codiceFase: string, numeroProgressivo: string): Observable<IDictionary<any>> {
        ...
        this.routerService.navigateToPage('modifica-fase-variazione-aggiudicatari-page', params);
        return new Observable((ob: Observer<IDictionary<any>>) => {
            ob.next(undefined);
            ob.complete();
        });
    ...
    private insertFaseFactory(insertForm: FaseVariazioneAggiudicatariInsertForm) {
        return () => {
            return this.faseVariazioneAggiudicatariService.insertFase(insertForm);
        }
    }

    private updateFaseFactory(insertForm: FaseVariazioneAggiudicatariInsertForm) {
        return () => {            
            return this.faseVariazioneAggiudicatariService.updateFase(insertForm);
        }
    }

    private getDefaultFaseForm(codGara: string, codLotto: string): FaseVariazioneAggiudicatariInsertForm {
        const request: FaseVariazioneAggiudicatariInsertForm = {
            codGara: +codGara,
            codLotto: +codLotto
        };
        return request;
    ...
    private elaborateOne(field: SdkFormBuilderField, request: FaseVariazioneAggiudicatariInsertForm, dynamicMappingOutput?: string): FaseVariazioneAggiudicatariInsertForm {
        if (isObject(field)) {
    ...
    private elaborateSection(field: SdkFormBuilderField, request: FaseVariazioneAggiudicatariInsertForm): FaseVariazioneAggiudicatariInsertForm {
    ...
    private elaborateGroup(field: SdkFormBuilderField, request: FaseVariazioneAggiudicatariInsertForm): FaseVariazioneAggiudicatariInsertForm {
    ...
    private populateRequest(formBuilderConfig: SdkFormBuilderConfiguration, codGara: string, codLotto: string): FaseVariazioneAggiudicatariInsertForm {
        let request: FaseVariazioneAggiudicatariInsertForm = this.getDefaultFaseForm(codGara, codLotto);
