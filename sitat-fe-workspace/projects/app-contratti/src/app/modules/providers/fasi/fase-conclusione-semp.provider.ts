...
import { FaseConclusioneSempInsertForm } from '../../models/fasi/fase-conclusione-semp.model';
...
export class FaseConclusioneSempProvider extends SdkBaseService implements SdkProvider {
    ...
    public run(args: IDictionary<any>): Observable<IDictionary<any>> {
        this.logger.debug('FaseConclusioneSempProvider >>>', args);
        ...
        if (args.buttonCode === 'back-to-dettaglio-fase') {
            ...
            if (valid === true) {
                ...
                const request: FaseConclusioneSempInsertForm = this.populateRequest(formBuilderConfig, codGara, codLotto);
                ...
                return this.requestHelper.begin(faseFactory, messagesPanel).pipe(
                    map((result: any) => {
                        ...
                        this.routerService.navigateToPage('dettaglio-fase-conclusione-semp-page', params);
                        return {
                            ...result,
    ...
    private modificaFase(codGara: string, codLotto: string, codiceFase: string, numeroProgressivo: string): Observable<IDictionary<any>> {
        ...
        this.routerService.navigateToPage('modifica-fase-conclusione-semp-page', params);
        return new Observable((ob: Observer<IDictionary<any>>) => {
            ob.next(undefined);
            ob.complete();
        });
    ...
    private insertFaseFactory(insertForm: FaseConclusioneSempInsertForm) {
        return () => {
            return this.faseConclusioneSempService.insertFase(insertForm);
        }
    }

    private updateFaseFactory(insertForm: FaseConclusioneSempInsertForm) {
        return () => {
            return this.faseConclusioneSempService.updateFase(insertForm);
        }
    }

    private getDefaultFaseForm(codGara: string, codLotto: string): FaseConclusioneSempInsertForm {
        const request: FaseConclusioneSempInsertForm = {
            codGara: +codGara,
            codLotto: +codLotto
        };
        return request;
    ...
    private elaborateOne(field: SdkFormBuilderField, request: FaseConclusioneSempInsertForm, dynamicMappingOutput?: string): FaseConclusioneSempInsertForm {
        if (isObject(field)) {
    ...
    private elaborateSection(field: SdkFormBuilderField, request: FaseConclusioneSempInsertForm): FaseConclusioneSempInsertForm {
    ...
    private elaborateGroup(field: SdkFormBuilderField, request: FaseConclusioneSempInsertForm): FaseConclusioneSempInsertForm {
    ...
    private populateRequest(formBuilderConfig: SdkFormBuilderConfiguration, codGara: string, codLotto: string): FaseConclusioneSempInsertForm {
        let request: FaseConclusioneSempInsertForm = this.getDefaultFaseForm(codGara, codLotto);
