...
import { each, get, head, isEmpty, isEqual, isFunction, isObject, isString, isUndefined, set } from 'lodash-es';
...
import { FaseComEsitoForm } from '../../models/fasi/fase-comunicazione-esito.model';
import { FaseComunicazioneEsitoService } from '../../services/fasi/fase-comunicazione.service';
import {
    NuovaComunicazioneEsitoValidationUtilsService,
} from '../../services/utils/nuova-com-esito-validation-utils.service';

@Injectable({ providedIn: 'root' })
export class FaseComunicazioneEsitoProvider extends SdkBaseService implements SdkProvider {
    ...
    public run(args: IDictionary<any>): Observable<IDictionary<any>> {
        this.logger.debug('FaseComunicazioneEsitoProvider >>>', args);
        ...
        if (args.buttonCode === 'back-to-dettaglio-fase') {
            ...
            if (!isEqual(defaultFormBuilderConfig, formBuilderConfig)) {
                ...
                let valid: boolean = this.nuovaComunicazioneEsitoValidationUtilsService.executeValidations(formBuilderConfig, messagesPanel);
                if (valid === true) {
                    // genero l'oggetto di richiesta
                    let request: FaseComEsitoForm = this.populateRequest(formBuilderConfig, codGara, codLotto);
                    ...
                    return this.requestHelper.begin(faseFactory, messagesPanel).pipe(
                        map((result: any) => {
                            ...
                            this.dettaglioFaseStoreService.numeroProgressivo = '1';
                            ...
                            this.routerService.navigateToPage('dettaglio-comunicazione-esito-page', params);
                            return {
                                ...result,
    ...
    private populateRequest(formBuilderConfig: SdkFormBuilderConfiguration, codGara: string, codLotto: string): FaseComEsitoForm {
        let request: FaseComEsitoForm = this.getDefaultFaseForm(codGara, codLotto);
    ...
    private getDefaultFaseForm(codGara: string, codLotto: string): FaseComEsitoForm {
        let request: FaseComEsitoForm = {
            codGara: +codGara,
            codLotto: +codLotto
        };
        return request;
    ...
    private elaborateOne(field: SdkFormBuilderField, request: FaseComEsitoForm, dynamicMappingOutput?: string): FaseComEsitoForm {
        if (isObject(field)) {
            if (!isEmpty(dynamicMappingOutput)) {
                ...
                if (field.type === 'COMBOBOX') {
                    ...
                    if (!isEmpty(field.documents)) {
                        let first: SdkDocumentItem = head(field.documents);
                        set(request, field.mappingOutput, first.binary);
                    }
                } else {
    ...
    private elaborateSection(field: SdkFormBuilderField, request: FaseComEsitoForm): FaseComEsitoForm {
    ...
    private elaborateGroup(field: SdkFormBuilderField, request: FaseComEsitoForm): FaseComEsitoForm {
    ...
    private insertFaseFactory(insertForm: FaseComEsitoForm) {
        return () => {
            return this.faseComunicazioneEsitoService.insertFase(insertForm);
        }
    }

    private updateFaseFactory(insertForm: FaseComEsitoForm) {
        return () => {
            return this.faseComunicazioneEsitoService.updateFase(insertForm);
        }
    }
    ...
    private modificaFase(codGara: string, codLotto: string, codiceFase: string, numeroProgressivo: string): Observable<IDictionary<any>> {
        ...
        this.routerService.navigateToPage('modifica-comunicazione-esito-page', params);
        return new Observable((ob: Observer<IDictionary<any>>) => {
            ob.next(undefined);
            ob.complete();
        });
    ...
    private get nuovaComunicazioneEsitoValidationUtilsService(): NuovaComunicazioneEsitoValidationUtilsService { return this.injectable(NuovaComunicazioneEsitoValidationUtilsService) }
