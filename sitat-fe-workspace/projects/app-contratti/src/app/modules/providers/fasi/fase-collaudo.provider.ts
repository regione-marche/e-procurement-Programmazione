...
import { FaseCollaudoInsertForm } from '../../models/fasi/fase-collaudo.model';
...
export class FaseCollaudoProvider extends SdkBaseService implements SdkProvider {
    ...
    public run(args: IDictionary<any>): Observable<IDictionary<any>> {
        ...
        if (args.buttonCode === 'back-to-dettaglio-fase') {
            ...
            if (valid === true) {
                ...
                const request: FaseCollaudoInsertForm = this.populateRequest(formBuilderConfig, codGara, codLotto);
                ...
                return this.requestHelper.begin(faseFactory, messagesPanel).pipe(
                    map((result: any) => {
                        ...
                        this.routerService.navigateToPage('dettaglio-fase-collaudo-page', params);
                        return {
                            ...result,
    ...
    private modificaFase(codGara: string, codLotto: string, codiceFase: string, numeroProgressivo: string): Observable<IDictionary<any>> {
        ...
        this.routerService.navigateToPage('modifica-fase-collaudo-page', params);
        return new Observable((ob: Observer<IDictionary<any>>) => {
            ob.next(undefined);
            ob.complete();
        });
    ...
    private insertFaseFactory(insertForm: FaseCollaudoInsertForm) {
        return () => {
            return this.faseCollaudoService.insertFase(insertForm);
        }
    }

    private updateFaseFactory(insertForm: FaseCollaudoInsertForm) {
        return () => {            
            return this.faseCollaudoService.updateFase(insertForm);
        }
    }

    private getDefaultFaseForm(codGara: string, codLotto: string): FaseCollaudoInsertForm {
        const request: FaseCollaudoInsertForm = {
            codGara: +codGara,
            codLotto: +codLotto
        };
        return request;
    ...
    private elaborateOne(field: SdkFormBuilderField, request: FaseCollaudoInsertForm, dynamicMappingOutput?: string): FaseCollaudoInsertForm {
        if (isObject(field)) {
    ...
    private elaborateSection(field: SdkFormBuilderField, request: FaseCollaudoInsertForm): FaseCollaudoInsertForm {
    ...
    private elaborateGroup(field: SdkFormBuilderField, request: FaseCollaudoInsertForm): FaseCollaudoInsertForm {
    ...
    private populateRequest(formBuilderConfig: SdkFormBuilderConfiguration, codGara: string, codLotto: string): FaseCollaudoInsertForm {
        let request: FaseCollaudoInsertForm = this.getDefaultFaseForm(codGara, codLotto);
