...
import { FaseInfortuniInsertForm } from '../../models/fasi/fase-infortuni.model';
...
export class FaseInfortuniProvider extends SdkBaseService implements SdkProvider {
    ...
    public run(args: IDictionary<any>): Observable<IDictionary<any>> {
        this.logger.debug('FaseInfortuniProvider >>>', args);
        ...
        if (args.buttonCode === 'back-to-dettaglio-fase') {
            ...
            if (valid === true) {
                ...
                const request: FaseInfortuniInsertForm = this.populateRequest(formBuilderConfig, codGara, codLotto);
                ...
                return this.requestHelper.begin(faseFactory, messagesPanel).pipe(
                    map((result: any) => {
                        ...
                        this.routerService.navigateToPage('dettaglio-fase-infortuni-page', params);
                        return {
                            ...result,
    ...
    private modificaFase(codGara: string, codLotto: string, codiceFase: string, numeroProgressivo: string): Observable<IDictionary<any>> {
        ...
        this.routerService.navigateToPage('modifica-fase-infortuni-page', params);
        return new Observable((ob: Observer<IDictionary<any>>) => {
            ob.next(undefined);
            ob.complete();
        });
    ...
    private insertFaseFactory(insertForm: FaseInfortuniInsertForm) {
        return () => {
            return this.faseInfortuniService.insertFase(insertForm);
        }
    }

    private updateFaseFactory(insertForm: FaseInfortuniInsertForm) {
        return () => {
            return this.faseInfortuniService.updateFase(insertForm);
        }
    }

    private getDefaultFaseForm(codGara: string, codLotto: string): FaseInfortuniInsertForm {
        const request: FaseInfortuniInsertForm = {
            codGara: +codGara,
            codLotto: +codLotto
        };
        return request;
    ...
    private elaborateOne(field: SdkFormBuilderField, request: FaseInfortuniInsertForm, dynamicMappingOutput?: string): FaseInfortuniInsertForm {
        if (isObject(field)) {
    ...
    private elaborateSection(field: SdkFormBuilderField, request: FaseInfortuniInsertForm): FaseInfortuniInsertForm {
    ...
    private elaborateGroup(field: SdkFormBuilderField, request: FaseInfortuniInsertForm): FaseInfortuniInsertForm {
    ...
    private populateRequest(formBuilderConfig: SdkFormBuilderConfiguration, codGara: string, codLotto: string): FaseInfortuniInsertForm {
        let request: FaseInfortuniInsertForm = this.getDefaultFaseForm(codGara, codLotto);
    ...
    private get faseInfortuniService(): FaseInfortuniService {
        return this.injectable(FaseInfortuniService);
    }
