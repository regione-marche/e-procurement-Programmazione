...
import { FaseSuperamentoQuartoContrattualeInsertForm } from '../../models/fasi/fase-sospensione.model';
...
export class FaseSuperamentoQuartoContrattualeProvider extends SdkBaseService implements SdkProvider {
    ...
    public run(args: IDictionary<any>): Observable<IDictionary<any>> {
        this.logger.debug('FaseSuperamentoQuartoContrattualeProvider >>>', args);
        ...
        if (args.buttonCode === 'back-to-dettaglio-fase') {
            ...
            if (valid === true) {
                ...
                let request: FaseSuperamentoQuartoContrattualeInsertForm = this.populateRequest(formBuilderConfig, codGara, codLotto);
                ...
                return this.requestHelper.begin(faseFactory, messagesPanel).pipe(
                    map((result: any) => {
                        ...
                        this.routerService.navigateToPage('dettaglio-fase-superamento-quarto-contrattuale-page', params);
                        return {
                            ...result,
    ...
    private modificaFase(codGara: string, codLotto: string, codiceFase: string, numeroProgressivo: string): Observable<IDictionary<any>> {
        ...
        this.routerService.navigateToPage('modifica-fase-superamento-quarto-contrattuale-page', params);
        return new Observable((ob: Observer<IDictionary<any>>) => {
            ob.next(undefined);
            ob.complete();
        });
    ...
    private insertFaseFactory(insertForm: FaseSuperamentoQuartoContrattualeInsertForm) {
        return () => {
            return this.faseSuperamentoQuartoContrattualeService.insertFase(insertForm);
        }
    }

    private updateFaseFactory(insertForm: FaseSuperamentoQuartoContrattualeInsertForm) {
        return () => {
            return this.faseSuperamentoQuartoContrattualeService.updateFase(insertForm);
        }
    }

    private getDefaultFaseForm(codGara: string, codLotto: string): FaseSuperamentoQuartoContrattualeInsertForm {
        const request: FaseSuperamentoQuartoContrattualeInsertForm = {
            codGara: +codGara,
            codLotto: +codLotto
        };
        return request;
    ...
    private elaborateOne(field: SdkFormBuilderField, request: FaseSuperamentoQuartoContrattualeInsertForm, dynamicMappingOutput?: string): FaseSuperamentoQuartoContrattualeInsertForm {
        if (isObject(field)) {
    ...
    private elaborateSection(field: SdkFormBuilderField, request: FaseSuperamentoQuartoContrattualeInsertForm): FaseSuperamentoQuartoContrattualeInsertForm {
    ...
    private elaborateGroup(field: SdkFormBuilderField, request: FaseSuperamentoQuartoContrattualeInsertForm): FaseSuperamentoQuartoContrattualeInsertForm {
    ...
    private populateRequest(formBuilderConfig: SdkFormBuilderConfiguration, codGara: string, codLotto: string): FaseSuperamentoQuartoContrattualeInsertForm {
        let request: FaseSuperamentoQuartoContrattualeInsertForm = this.getDefaultFaseForm(codGara, codLotto);
