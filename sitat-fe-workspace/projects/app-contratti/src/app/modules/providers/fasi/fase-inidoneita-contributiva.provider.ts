...
import { FaseInidoneitaContributivaInsertForm } from '../../models/fasi/fase-inidoneita-contributiva.model';
...
export class FaseInidoneitaContributivaProvider extends SdkBaseService implements SdkProvider {
    ...
    public run(args: IDictionary<any>): Observable<IDictionary<any>> {
        this.logger.debug('FaseInidoneitaContributivaProvider >>>', args);
        ...
        if (args.buttonCode === 'back-to-dettaglio-fase') {
            ...
            if (valid === true) {
                ...
                const request: FaseInidoneitaContributivaInsertForm = this.populateRequest(formBuilderConfig, codGara, codLotto);
                ...
                return this.requestHelper.begin(faseFactory, messagesPanel).pipe(
                    map((result: any) => {
                        ...
                        this.routerService.navigateToPage('dettaglio-fase-inidoneita-contributiva-page', params);
                        return {
                            ...result,
    ...
    private modificaFase(codGara: string, codLotto: string, codiceFase: string, numeroProgressivo: string): Observable<IDictionary<any>> {
        ...
        this.routerService.navigateToPage('modifica-fase-inidoneita-contributiva-page', params);
        return new Observable((ob: Observer<IDictionary<any>>) => {
            ob.next(undefined);
            ob.complete();
        });
    ...
    private insertFaseFactory(insertForm: FaseInidoneitaContributivaInsertForm) {
        return () => {
            return this.faseInidoneitaContributivaService.insertFase(insertForm);
        }
    }

    private updateFaseFactory(insertForm: FaseInidoneitaContributivaInsertForm) {
        return () => {
            return this.faseInidoneitaContributivaService.updateFase(insertForm);
        }
    }

    private getDefaultFaseForm(codGara: string, codLotto: string): FaseInidoneitaContributivaInsertForm {
        const request: FaseInidoneitaContributivaInsertForm = {
            codGara: +codGara,
            codLotto: +codLotto
        };
        return request;
    ...
    private elaborateOne(field: SdkFormBuilderField, request: FaseInidoneitaContributivaInsertForm, dynamicMappingOutput?: string): FaseInidoneitaContributivaInsertForm {
        if (isObject(field)) {
    ...
    private elaborateSection(field: SdkFormBuilderField, request: FaseInidoneitaContributivaInsertForm): FaseInidoneitaContributivaInsertForm {
    ...
    private elaborateGroup(field: SdkFormBuilderField, request: FaseInidoneitaContributivaInsertForm): FaseInidoneitaContributivaInsertForm {
    ...
    private populateRequest(formBuilderConfig: SdkFormBuilderConfiguration, codGara: string, codLotto: string): FaseInidoneitaContributivaInsertForm {
        let request: FaseInidoneitaContributivaInsertForm = this.getDefaultFaseForm(codGara, codLotto);
