...
import { FaseAggInsertForm } from '../../models/fasi/fase-aggiudicazione.model';
...
export class FaseAggiudicazioneProvider extends SdkBaseService implements SdkProvider {
    ...
    public run(args: IDictionary<any>): Observable<IDictionary<any>> {
        this.logger.debug('FaseAggiudicazioneProvider >>>', args);
        ...
        if (args.buttonCode === 'back-to-dettaglio-fase') {
            ...
            const riaggiudicazione: boolean = args.riaggiudicazione;
            ...
            if (!isEqual(defaultFormBuilderConfig, formBuilderConfig)) {
                ...
                if (valid === true) {
                    ...
                    let request: FaseAggInsertForm = this.populateRequest(formBuilderConfig, codGara, codLotto);
                    ...
                    return this.requestHelper.begin(faseFactory, messagesPanel).pipe(
                        map((result: any) => {
                            ...
                            this.dettaglioFaseStoreService.riaggiudicazione = riaggiudicazione;
                            ...
                            this.routerService.navigateToPage('dettaglio-fase-aggiudicazione-page', params);
                            return {
                                ...result,
    ...
    private populateRequest(formBuilderConfig: SdkFormBuilderConfiguration, codGara: string, codLotto: string): FaseAggInsertForm {
        let request: FaseAggInsertForm = this.getDefaultFaseForm(codGara, codLotto);
    ...
    private getDefaultFaseForm(codGara: string, codLotto: string): FaseAggInsertForm {
        let request: FaseAggInsertForm = {
            codGara: +codGara,
            codLotto: +codLotto
        };
        return request;
    ...
    private elaborateOne(field: SdkFormBuilderField, request: FaseAggInsertForm, dynamicMappingOutput?: string): FaseAggInsertForm {
        if (isObject(field)) {
            if (!isEmpty(dynamicMappingOutput)) {
                ...
                if (field.type === 'COMBOBOX') {
                    ...
                    if (!isUndefined(field.data)) {
                        let data: Date = get(field, 'data') as Date;
                        if (isObject(data)) {
    ...
    private elaborateSection(field: SdkFormBuilderField, request: FaseAggInsertForm): FaseAggInsertForm {
    ...
    private elaborateGroup(field: SdkFormBuilderField, request: FaseAggInsertForm): FaseAggInsertForm {
    ...
    private insertFaseFactory(insertForm: FaseAggInsertForm) {
        return () => {
            return this.faseAggiudicazioneService.insertFase(insertForm);
        }
    }

    private updateFaseFactory(insertForm: FaseAggInsertForm) {
        return () => {
            return this.faseAggiudicazioneService.updateFase(insertForm);
        }
    }
    ...
    private modificaFase(codGara: string, codLotto: string, codiceFase: string, numeroProgressivo: string): Observable<IDictionary<any>> {
        ...
        this.routerService.navigateToPage('modifica-fase-aggiudicazione-page', params);
        return new Observable((ob: Observer<IDictionary<any>>) => {
            ob.next(undefined);
            ob.complete();
        });
