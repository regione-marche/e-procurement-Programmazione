...
import { FaseConclusioneInsertForm } from '../../models/fasi/fase-conclusione.model';
...
export class FaseConclusioneProvider extends SdkBaseService implements SdkProvider {
    ...
    public run(args: IDictionary<any>): Observable<IDictionary<any>> {
        ...
        if (args.buttonCode === 'back-to-dettaglio-fase') {
            ...
            if (valid === true) {
                ...
                const request: FaseConclusioneInsertForm = this.populateRequest(formBuilderConfig, codGara, codLotto);
                ...
                return this.requestHelper.begin(faseFactory, messagesPanel).pipe(
                    map((result: any) => {
                        ...
                        this.routerService.navigateToPage('dettaglio-fase-conclusione-page', params);
                        return {
                            ...result,
    ...
    private modificaFase(codGara: string, codLotto: string, codiceFase: string, numeroProgressivo: string): Observable<IDictionary<any>> {
        ...
        this.routerService.navigateToPage('modifica-fase-conclusione-page', params);
        return new Observable((ob: Observer<IDictionary<any>>) => {
            ob.next(undefined);
            ob.complete();
        });
    ...
    private insertFaseFactory(insertForm: FaseConclusioneInsertForm) {
        return () => {
            return this.faseConclusioneService.insertFase(insertForm);
        }
    }

    private updateFaseFactory(insertForm: FaseConclusioneInsertForm) {
        return () => {
            return this.faseConclusioneService.updateFase(insertForm);
        }
    }

    private getDefaultFaseForm(codGara: string, codLotto: string): FaseConclusioneInsertForm {
        const request: FaseConclusioneInsertForm = {
            codGara: +codGara,
            codLotto: +codLotto
        };
        return request;
    ...
    private elaborateOne(field: SdkFormBuilderField, request: FaseConclusioneInsertForm, dynamicMappingOutput?: string): FaseConclusioneInsertForm {
        if (isObject(field)) {
    ...
    private elaborateSection(field: SdkFormBuilderField, request: FaseConclusioneInsertForm): FaseConclusioneInsertForm {
    ...
    private elaborateGroup(field: SdkFormBuilderField, request: FaseConclusioneInsertForm): FaseConclusioneInsertForm {
    ...
    private populateRequest(formBuilderConfig: SdkFormBuilderConfiguration, codGara: string, codLotto: string): FaseConclusioneInsertForm {
        let request: FaseConclusioneInsertForm = this.getDefaultFaseForm(codGara, codLotto);
