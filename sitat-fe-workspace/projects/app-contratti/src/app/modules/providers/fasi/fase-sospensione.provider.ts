...
import { FaseSospensioneInsertForm } from '../../models/fasi/fase-sospensione.model';
...
export class FaseSospensioneProvider extends SdkBaseService implements SdkProvider {
    ...
    public run(args: IDictionary<any>): Observable<IDictionary<any>> {
        this.logger.debug('FaseSospensioneProvider >>>', args);
        ...
        if (args.buttonCode === 'back-to-dettaglio-fase') {
            ...
            if (valid === true) {
                ...
                const request: FaseSospensioneInsertForm = this.populateRequest(formBuilderConfig, codGara, codLotto);
                ...
                return this.requestHelper.begin(faseFactory, messagesPanel).pipe(
                    map((result: any) => {
                        ...
                        this.routerService.navigateToPage('dettaglio-fase-sospensione-page', params);
                        return {
                            ...result,
    ...
    private modificaFase(codGara: string, codLotto: string, codiceFase: string, numeroProgressivo: string): Observable<IDictionary<any>> {
        ...
        this.routerService.navigateToPage('modifica-fase-sospensione-page', params);
        return new Observable((ob: Observer<IDictionary<any>>) => {
            ob.next(undefined);
            ob.complete();
        });
    ...
    private insertFaseFactory(insertForm: FaseSospensioneInsertForm) {
        return () => {
            return this.faseSospensioneService.insertFase(insertForm);
        }
    }

    private updateFaseFactory(insertForm: FaseSospensioneInsertForm) {
        return () => {
            return this.faseSospensioneService.updateFase(insertForm);
        }
    }

    private getDefaultFaseForm(codGara: string, codLotto: string): FaseSospensioneInsertForm {
        const request: FaseSospensioneInsertForm = {
            codGara: +codGara,
            codLotto: +codLotto
        };
        return request;
    ...
    private elaborateOne(field: SdkFormBuilderField, request: FaseSospensioneInsertForm, dynamicMappingOutput?: string): FaseSospensioneInsertForm {
        if (isObject(field)) {
    ...
    private elaborateSection(field: SdkFormBuilderField, request: FaseSospensioneInsertForm): FaseSospensioneInsertForm {
    ...
    private elaborateGroup(field: SdkFormBuilderField, request: FaseSospensioneInsertForm): FaseSospensioneInsertForm {
    ...
    private populateRequest(formBuilderConfig: SdkFormBuilderConfiguration, codGara: string, codLotto: string): FaseSospensioneInsertForm {
        let request: FaseSospensioneInsertForm = this.getDefaultFaseForm(codGara, codLotto);
