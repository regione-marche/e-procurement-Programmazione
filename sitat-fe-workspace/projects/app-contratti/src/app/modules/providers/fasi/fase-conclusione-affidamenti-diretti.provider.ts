...
import { FaseConclusioneAffidamentiDirettiInsertForm } from '../../models/fasi/fase-conclusione-affidamenti-diretti.model';
import { AssociazionePagamentiEntry } from '../../models/gare/gare.model';


@Injectable({ providedIn: 'root' })
export class FaseConclusioneAffidamentiDirettiProvider extends SdkBaseService implements SdkProvider {
    ...
    public run(args: IDictionary<any>): Observable<IDictionary<any>> {
        this.logger.debug('FaseConclusioneProvider >>>', args);
        ...
        if (args.buttonCode === 'back-to-dettaglio-fase') {
            ...
            const associazioniPagamenti: Array<AssociazionePagamentiEntry> = args.associazioniPagamenti;
            ...
            if (valid === true) {
                ...
                const request: FaseConclusioneAffidamentiDirettiInsertForm = this.populateRequest(formBuilderConfig, codGara, codLotto);
                request.associazioniPagamenti = associazioniPagamenti;
                ...
                return this.requestHelper.begin(faseFactory, messagesPanel).pipe(
                    map((result: any) => {
                        ...
                        this.routerService.navigateToPage('dettaglio-fase-conclusione-affidamenti-diretti-page', params);
                        return {
                            ...result,
    ...
    private modificaFase(codGara: string, codLotto: string, codiceFase: string, numeroProgressivo: string): Observable<IDictionary<any>> {
        ...
        this.routerService.navigateToPage('modifica-fase-conclusione-affidamenti-diretti-page', params);
        return new Observable((ob: Observer<IDictionary<any>>) => {
            ob.next(undefined);
            ob.complete();
        });
    ...
    private insertFaseFactory(insertForm: FaseConclusioneAffidamentiDirettiInsertForm) {
        return () => {
            return this.faseConclusioneAffidamentiDirettiService.insertFase(insertForm);
        }
    }

    private updateFaseFactory(insertForm: FaseConclusioneAffidamentiDirettiInsertForm) {
        return () => {
            return this.faseConclusioneAffidamentiDirettiService.updateFase(insertForm);
        }
    }

    private getDefaultFaseForm(codGara: string, codLotto: string): FaseConclusioneAffidamentiDirettiInsertForm {
        const request: FaseConclusioneAffidamentiDirettiInsertForm = {
            codGara: +codGara,
            codLotto: +codLotto
        };
        return request;
    ...
    private elaborateOne(field: SdkFormBuilderField, request: FaseConclusioneAffidamentiDirettiInsertForm, dynamicMappingOutput?: string): FaseConclusioneAffidamentiDirettiInsertForm {
        if (isObject(field)) {
    ...
    private elaborateSection(field: SdkFormBuilderField, request: FaseConclusioneAffidamentiDirettiInsertForm): FaseConclusioneAffidamentiDirettiInsertForm {
    ...
    private elaborateGroup(field: SdkFormBuilderField, request: FaseConclusioneAffidamentiDirettiInsertForm): FaseConclusioneAffidamentiDirettiInsertForm {
    ...
    private populateRequest(formBuilderConfig: SdkFormBuilderConfiguration, codGara: string, codLotto: string): FaseConclusioneAffidamentiDirettiInsertForm {
        let request: FaseConclusioneAffidamentiDirettiInsertForm = this.getDefaultFaseForm(codGara, codLotto);
