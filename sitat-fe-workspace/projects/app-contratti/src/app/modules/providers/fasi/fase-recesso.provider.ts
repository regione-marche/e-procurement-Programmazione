...
import { FaseIstanzaRecessoInsertForm } from '../../models/fasi/fase-recesso.model';
...
export class FaseRecessoProvider extends SdkBaseService implements SdkProvider {
    ...
    public run(args: IDictionary<any>): Observable<IDictionary<any>> {
        this.logger.debug('FaseRecessoProvider >>>', args);
        ...
        if (args.buttonCode === 'back-to-dettaglio-fase') {
            ...
            if (valid === true) {
                ...
                const request: FaseIstanzaRecessoInsertForm = this.populateRequest(formBuilderConfig, codGara, codLotto);
                ...
                return this.requestHelper.begin(faseFactory, messagesPanel).pipe(
                    map((result: any) => {
                        ...
                        this.routerService.navigateToPage('dettaglio-fase-recesso-page', params);
                        return {
                            ...result,
    ...
    private modificaFase(codGara: string, codLotto: string, codiceFase: string, numeroProgressivo: string): Observable<IDictionary<any>> {
        ...
        this.routerService.navigateToPage('modifica-fase-recesso-page', params);
        return new Observable((ob: Observer<IDictionary<any>>) => {
            ob.next(undefined);
            ob.complete();
        });
    ...
    private insertFaseFactory(insertForm: FaseIstanzaRecessoInsertForm) {
        return () => {
            return this.faseRecessoService.insertFase(insertForm);
        }
    }

    private updateFaseFactory(insertForm: FaseIstanzaRecessoInsertForm) {
        return () => {
            return this.faseRecessoService.updateFase(insertForm);
        }
    }

    private getDefaultFaseForm(codGara: string, codLotto: string): FaseIstanzaRecessoInsertForm {
        const request: FaseIstanzaRecessoInsertForm = {
            codGara: +codGara,
            codLotto: +codLotto
        };
        return request;
    ...
    private elaborateOne(field: SdkFormBuilderField, request: FaseIstanzaRecessoInsertForm, dynamicMappingOutput?: string): FaseIstanzaRecessoInsertForm {
        if (isObject(field)) {
    ...
    private elaborateSection(field: SdkFormBuilderField, request: FaseIstanzaRecessoInsertForm): FaseIstanzaRecessoInsertForm {
    ...
    private elaborateGroup(field: SdkFormBuilderField, request: FaseIstanzaRecessoInsertForm): FaseIstanzaRecessoInsertForm {
    ...
    private populateRequest(formBuilderConfig: SdkFormBuilderConfiguration, codGara: string, codLotto: string): FaseIstanzaRecessoInsertForm {
        let request: FaseIstanzaRecessoInsertForm = this.getDefaultFaseForm(codGara, codLotto);
