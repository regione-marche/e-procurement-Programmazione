...
import { FaseAvanzamentoSempInsertForm } from '../../models/fasi/fase-avanzamento-semp.model';
...
export class FaseAvanzamentoSempProvider extends SdkBaseService implements SdkProvider {
    ...
    public run(args: IDictionary<any>): Observable<IDictionary<any>> {
        this.logger.debug('FaseAvanzamentoSempProvider >>>', args);
        ...
        if (args.buttonCode === 'back-to-dettaglio-fase') {
            ...
            if (valid === true) {
                ...
                const request: FaseAvanzamentoSempInsertForm = this.populateRequest(formBuilderConfig, codGara, codLotto);
                ...
                return this.requestHelper.begin(faseFactory, messagesPanel).pipe(
                    map((result: any) => {
                        ...
                        this.routerService.navigateToPage('dettaglio-fase-avanzamento-semp-page', params);
                        return {
                            ...result,
    ...
    private modificaFase(codGara: string, codLotto: string, codiceFase: string, numeroProgressivo: string): Observable<IDictionary<any>> {
        ...
        this.routerService.navigateToPage('modifica-fase-avanzamento-semp-page', params);
        return new Observable((ob: Observer<IDictionary<any>>) => {
            ob.next(undefined);
            ob.complete();
        });
    ...
    private insertFaseFactory(insertForm: FaseAvanzamentoSempInsertForm) {
        return () => {
            return this.faseAvanzamentoSempService.insertFase(insertForm);
        }
    }

    private updateFaseFactory(insertForm: FaseAvanzamentoSempInsertForm) {
        return () => {
            return this.faseAvanzamentoSempService.updateFase(insertForm);
        }
    }

    private getDefaultFaseForm(codGara: string, codLotto: string): FaseAvanzamentoSempInsertForm {
        const request: FaseAvanzamentoSempInsertForm = {
            codGara: +codGara,
            codLotto: +codLotto
        };
        return request;
    ...
    private elaborateOne(field: SdkFormBuilderField, request: FaseAvanzamentoSempInsertForm, dynamicMappingOutput?: string): FaseAvanzamentoSempInsertForm {
        if (isObject(field)) {
    ...
    private elaborateSection(field: SdkFormBuilderField, request: FaseAvanzamentoSempInsertForm): FaseAvanzamentoSempInsertForm {
    ...
    private elaborateGroup(field: SdkFormBuilderField, request: FaseAvanzamentoSempInsertForm): FaseAvanzamentoSempInsertForm {
    ...
    private populateRequest(formBuilderConfig: SdkFormBuilderConfiguration, codGara: string, codLotto: string): FaseAvanzamentoSempInsertForm {
        let request: FaseAvanzamentoSempInsertForm = this.getDefaultFaseForm(codGara, codLotto);
