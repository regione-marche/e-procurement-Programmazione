...
import { FaseRipresaPrestazioneInsertForm } from '../../models/fasi/fase-sospensione.model';
...
export class FaseRipresaPrestazioneProvider extends SdkBaseService implements SdkProvider {
    ...
    public run(args: IDictionary<any>): Observable<IDictionary<any>> {
        this.logger.debug('FaseRipresaPrestazioneProvider >>>', args);
        ...
        if (args.buttonCode === 'back-to-dettaglio-fase') {
            ...
            if (valid === true) {
                ...
                let request: FaseRipresaPrestazioneInsertForm = this.populateRequest(formBuilderConfig, codGara, codLotto);
                ...
                return this.requestHelper.begin(faseFactory, messagesPanel).pipe(
                    map((result: any) => {
                        ...
                        this.routerService.navigateToPage('dettaglio-fase-ripresa-prestazione-page', params);
                        return {
                            ...result,
    ...
    private modificaFase(codGara: string, codLotto: string, codiceFase: string, numeroProgressivo: string): Observable<IDictionary<any>> {
        ...
        this.routerService.navigateToPage('modifica-fase-ripresa-prestazione-page', params);
        return new Observable((ob: Observer<IDictionary<any>>) => {
            ob.next(undefined);
            ob.complete();
        });
    ...
    private insertFaseFactory(insertForm: FaseRipresaPrestazioneInsertForm) {
        return () => {
            return this.faseRipresaPrestazioneService.insertFase(insertForm);
        }
    }

    private updateFaseFactory(insertForm: FaseRipresaPrestazioneInsertForm) {
        return () => {
            return this.faseRipresaPrestazioneService.updateFase(insertForm);
        }
    }

    private getDefaultFaseForm(codGara: string, codLotto: string): FaseRipresaPrestazioneInsertForm {
        const request: FaseRipresaPrestazioneInsertForm = {
            codGara: +codGara,
            codLotto: +codLotto
        };
        return request;
    ...
    private elaborateOne(field: SdkFormBuilderField, request: FaseRipresaPrestazioneInsertForm, dynamicMappingOutput?: string): FaseRipresaPrestazioneInsertForm {
        if (isObject(field)) {
    ...
    private elaborateSection(field: SdkFormBuilderField, request: FaseRipresaPrestazioneInsertForm): FaseRipresaPrestazioneInsertForm {
    ...
    private elaborateGroup(field: SdkFormBuilderField, request: FaseRipresaPrestazioneInsertForm): FaseRipresaPrestazioneInsertForm {
    ...
    private populateRequest(formBuilderConfig: SdkFormBuilderConfiguration, codGara: string, codLotto: string): FaseRipresaPrestazioneInsertForm {
        let request: FaseRipresaPrestazioneInsertForm = this.getDefaultFaseForm(codGara, codLotto);
