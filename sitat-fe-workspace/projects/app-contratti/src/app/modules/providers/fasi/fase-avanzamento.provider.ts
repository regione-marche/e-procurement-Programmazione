...
import { FaseAvanzamentoInsertForm } from '../../models/fasi/fase-avanzamento.model';
...
export class FaseAvanzamentoProvider extends SdkBaseService implements SdkProvider {
    ...
    public run(args: IDictionary<any>): Observable<IDictionary<any>> {
        this.logger.debug('FaseAvanzamentoProvider >>>', args);
        ...
        if (args.buttonCode === 'back-to-dettaglio-fase') {
            ...
            if (valid === true) {
                ...
                const request: FaseAvanzamentoInsertForm = this.populateRequest(formBuilderConfig, codGara, codLotto);
                ...
                return this.requestHelper.begin(faseFactory, messagesPanel).pipe(
                    map((result: any) => {
                        ...
                        this.routerService.navigateToPage('dettaglio-fase-avanzamento-page', params);
                        return {
                            ...result,
    ...
    private modificaFase(codGara: string, codLotto: string, codiceFase: string, numeroProgressivo: string): Observable<IDictionary<any>> {
        ...
        this.routerService.navigateToPage('modifica-fase-avanzamento-page', params);
        return new Observable((ob: Observer<IDictionary<any>>) => {
            ob.next(undefined);
            ob.complete();
        });
    ...
    private insertFaseFactory(insertForm: FaseAvanzamentoInsertForm) {
        return () => {
            return this.faseAvanzamentoService.insertFase(insertForm);
        }
    }

    private updateFaseFactory(insertForm: FaseAvanzamentoInsertForm) {
        return () => {
            return this.faseAvanzamentoService.updateFase(insertForm);
        }
    }

    private getDefaultFaseForm(codGara: string, codLotto: string): FaseAvanzamentoInsertForm {
        let request: FaseAvanzamentoInsertForm = {
            codGara: +codGara,
            codLotto: +codLotto
        };
        return request;
    ...
    private elaborateOne(field: SdkFormBuilderField, request: FaseAvanzamentoInsertForm, dynamicMappingOutput?: string): FaseAvanzamentoInsertForm {
        if (isObject(field)) {
    ...
    private elaborateSection(field: SdkFormBuilderField, request: FaseAvanzamentoInsertForm): FaseAvanzamentoInsertForm {
    ...
    private elaborateGroup(field: SdkFormBuilderField, request: FaseAvanzamentoInsertForm): FaseAvanzamentoInsertForm {
    ...
    private populateRequest(formBuilderConfig: SdkFormBuilderConfiguration, codGara: string, codLotto: string): FaseAvanzamentoInsertForm {
        let request: FaseAvanzamentoInsertForm = this.getDefaultFaseForm(codGara, codLotto);
