...
import { FaseInizialeEsecuzioneInsertForm } from '../../models/fasi/fase-iniziale.model';
...
export class FaseInizialeSottoscrContrProvider extends SdkBaseService implements SdkProvider {
    ...
    public run(args: IDictionary<any>): Observable<IDictionary<any>> {
        this.logger.debug('FaseInizialeProvider >>>', args);
        ...
        if (args.buttonCode === 'back-to-dettaglio-fase') {
            ...
            if (!isEqual(defaultFormBuilderConfig, formBuilderConfig)) {
                ...
                if (valid === true) {
                    ...
                    let request: FaseInizialeEsecuzioneInsertForm = this.populateRequest(formBuilderConfig, codGara, codLotto);
                    ...
                    return this.requestHelper.begin(faseFactory, messagesPanel).pipe(
                        map((result: any) => {
                            ...
                            this.routerService.navigateToPage('dettaglio-fase-iniz-sottoscrizione-contratto-page', params);
                            return {
                                ...result,
    ...
    private populateRequest(formBuilderConfig: SdkFormBuilderConfiguration, codGara: string, codLotto: string): FaseInizialeEsecuzioneInsertForm {
        let request: FaseInizialeEsecuzioneInsertForm = this.getDefaultFaseForm(codGara, codLotto);
    ...
    private getDefaultFaseForm(codGara: string, codLotto: string): FaseInizialeEsecuzioneInsertForm {
        let request: FaseInizialeEsecuzioneInsertForm = {
            codGara: +codGara,
            codLotto: +codLotto,
            pubblicazioneEsito: {
                codGara: +codGara,
    ...
    private elaborateOne(field: SdkFormBuilderField, request: FaseInizialeEsecuzioneInsertForm, dynamicMappingOutput?: string): FaseInizialeEsecuzioneInsertForm {
        if (isObject(field)) {
    ...
    private elaborateSection(field: SdkFormBuilderField, request: FaseInizialeEsecuzioneInsertForm): FaseInizialeEsecuzioneInsertForm {
    ...
    private elaborateGroup(field: SdkFormBuilderField, request: FaseInizialeEsecuzioneInsertForm): FaseInizialeEsecuzioneInsertForm {
    ...
    private insertFaseFactory(insertForm: FaseInizialeEsecuzioneInsertForm) {
        return () => {
            return this.faseSottoScrizioneContrattoService.insertFase(insertForm);
        }
    }

    private updateFaseFactory(insertForm: FaseInizialeEsecuzioneInsertForm) {
        return () => {
            return this.faseSottoScrizioneContrattoService.updateFase(insertForm);
        }
    }
    ...
    private modificaFase(codGara: string, codLotto: string, codiceFase: string, numeroProgressivo: string): Observable<IDictionary<any>> {
        ...
        this.routerService.navigateToPage('modifica-fase-iniz-sottoscrizione-contratto-page', params);
        return new Observable((ob: Observer<IDictionary<any>>) => {
            ob.next(undefined);
            ob.complete();
        });
