...
import { FaseAccordoBonarioInsertForm } from '../../models/fasi/fase-accordo-bonario.model';
...
export class FaseAccordoBonarioProvider extends SdkBaseService implements SdkProvider {
    ...
    public run(args: IDictionary<any>): Observable<IDictionary<any>> {
        this.logger.debug('FaseAccordoBonarioProvider >>>', args);
        ...
        if (args.buttonCode === 'back-to-dettaglio-fase') {
            ...
            if (valid === true) {
                ...
                const request: FaseAccordoBonarioInsertForm = this.populateRequest(formBuilderConfig, codGara, codLotto);
                ...
                return this.requestHelper.begin(faseFactory, messagesPanel).pipe(
                    map((result: any) => {
                        ...
                        this.routerService.navigateToPage('dettaglio-fase-accordo-bonario-page', params);
                        return {
                            ...result,
    ...
    private modificaFase(codGara: string, codLotto: string, codiceFase: string, numeroProgressivo: string): Observable<IDictionary<any>> {
        ...
        this.routerService.navigateToPage('modifica-fase-accordo-bonario-page', params);
        return new Observable((ob: Observer<IDictionary<any>>) => {
            ob.next(undefined);
            ob.complete();
        });
    ...
    private insertFaseFactory(insertForm: FaseAccordoBonarioInsertForm) {
        return () => {
            return this.faseAccordoBonarioService.insertFase(insertForm);
        }
    }

    private updateFaseFactory(insertForm: FaseAccordoBonarioInsertForm) {
        return () => {
            return this.faseAccordoBonarioService.updateFase(insertForm);
        }
    }

    private getDefaultFaseForm(codGara: string, codLotto: string): FaseAccordoBonarioInsertForm {
        const request: FaseAccordoBonarioInsertForm = {
            codGara: +codGara,
            codLotto: +codLotto
        };
        return request;
    ...
    private elaborateOne(field: SdkFormBuilderField, request: FaseAccordoBonarioInsertForm, dynamicMappingOutput?: string): FaseAccordoBonarioInsertForm {
        if (isObject(field)) {
    ...
    private elaborateSection(field: SdkFormBuilderField, request: FaseAccordoBonarioInsertForm): FaseAccordoBonarioInsertForm {
    ...
    private elaborateGroup(field: SdkFormBuilderField, request: FaseAccordoBonarioInsertForm): FaseAccordoBonarioInsertForm {
    ...
    private populateRequest(formBuilderConfig: SdkFormBuilderConfiguration, codGara: string, codLotto: string): FaseAccordoBonarioInsertForm {
        let request: FaseAccordoBonarioInsertForm = this.getDefaultFaseForm(codGara, codLotto);
