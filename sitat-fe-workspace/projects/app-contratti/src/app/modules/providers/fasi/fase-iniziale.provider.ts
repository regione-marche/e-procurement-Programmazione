...
export class FaseInizialeProvider extends SdkBaseService implements SdkProvider {
                ...
                if (valid === true) {
                    ...
                    return this.requestHelper.begin(faseFactory, messagesPanel).pipe(
                        map((result: any) => {
                            ...
                            this.routerService.navigateToPage('dettaglio-fase-iniziale-page', params);
                            return {
                                ...result,
    ...
    private insertFaseFactory(insertForm: FaseInizialeEsecuzioneInsertForm) {
        return () => {
            return this.faseInizialeService.insertFase(insertForm);
        }
    }

    private updateFaseFactory(insertForm: FaseInizialeEsecuzioneInsertForm) {
        return () => {
            return this.faseInizialeService.updateFase(insertForm);
        }
    }
    ...
    private modificaFase(codGara: string, codLotto: string, codiceFase: string, numeroProgressivo: string): Observable<IDictionary<any>> {
        ...
        this.routerService.navigateToPage('modifica-fase-iniziale-page', params);
        return new Observable((ob: Observer<IDictionary<any>>) => {
            ob.next(undefined);
            ob.complete();
        });
