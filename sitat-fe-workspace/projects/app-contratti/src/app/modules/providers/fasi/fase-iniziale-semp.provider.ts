...
import { FaseInizialeSemplificataInsertForm } from '../../models/fasi/fase-iniziale-semp.model';
...
export class FaseInizialeSempProvider extends SdkBaseService implements SdkProvider {
    ...
    public run(args: IDictionary<any>): Observable<IDictionary<any>> {
        this.logger.debug('FaseInizialeSempProvider >>>', args);
        ...
        if (args.buttonCode === 'back-to-dettaglio-fase') {
            ...
            if (valid === true) {
                ...
                let request: FaseInizialeSemplificataInsertForm = this.populateRequest(formBuilderConfig, codGara, codLotto);
                ...
                return this.requestHelper.begin(faseFactory, messagesPanel).pipe(
                    map((result: any) => {
                        ...
                        this.routerService.navigateToPage('dettaglio-fase-iniziale-semp-page', params);
                        return {
                            ...result,
    ...
    private modificaFase(codGara: string, codLotto: string, codiceFase: string, numeroProgressivo: string): Observable<IDictionary<any>> {
        ...
        this.routerService.navigateToPage('modifica-fase-iniziale-semp-page', params);
        return new Observable((ob: Observer<IDictionary<any>>) => {
            ob.next(undefined);
            ob.complete();
        });
    ...
    private insertFaseFactory(insertForm: FaseInizialeSemplificataInsertForm) {
        return () => {
            return this.faseInizialeSempService.insertFase(insertForm);
        }
    }

    private updateFaseFactory(insertForm: FaseInizialeSemplificataInsertForm) {
        return () => {
            return this.faseInizialeSempService.updateFase(insertForm);
        }
    }

    private getDefaultFaseForm(codGara: string, codLotto: string): FaseInizialeSemplificataInsertForm {
        const request: FaseInizialeSemplificataInsertForm = {
            codGara: +codGara,
            codLotto: +codLotto
        };
        return request;
    ...
    private elaborateOne(field: SdkFormBuilderField, request: FaseInizialeSemplificataInsertForm, dynamicMappingOutput?: string): FaseInizialeSemplificataInsertForm {
        if (isObject(field)) {
    ...
    private elaborateSection(field: SdkFormBuilderField, request: FaseInizialeSemplificataInsertForm): FaseInizialeSemplificataInsertForm {
    ...
    private elaborateGroup(field: SdkFormBuilderField, request: FaseInizialeSemplificataInsertForm): FaseInizialeSemplificataInsertForm {
    ...
    private populateRequest(formBuilderConfig: SdkFormBuilderConfiguration, codGara: string, codLotto: string): FaseInizialeSemplificataInsertForm {
        let request: FaseInizialeSemplificataInsertForm = this.getDefaultFaseForm(codGara, codLotto);
