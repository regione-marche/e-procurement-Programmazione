...
import { cloneDeep, get, isEmpty, isObject, isString, map, toString } from 'lodash-es';
...
@Component({
    templateUrl: `lista-tecnici-modal.widget.html`,
    encapsulation: ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.OnPush
})
export class ListaTecniciModalWidget extends SdkAbstractComponent<any, void, any> implements OnInit, AfterViewInit, OnDestroy {
    ...
    private listaTecnici: Array<BaseRupInfo>;
    ...
    protected onConfig(config: any): void {
        if (isObject(config)) {
            this.markForCheck(() => {
                this.config = { ...config };
                this.listaTecnici = this.config.listaTecnici;
                this.isReady = true;
            });
        }
    }
        ...
        if (isObject(this.config)) {
            this.markForCheck(() => {
                let formConfig: SdkFormBuilderConfiguration = {
                    fields: get(this.config, 'listaTecniciData.fields')
                };
                ...
                let customPopulateFunction: CustomParamsFunction = (field: SdkFormBuilderField, restObject: any) => {
                    ...
                    if (field.code === 'lista-tecnici') {
                        field.itemsProvider = () => {
                            return new Observable<Array<SdkComboBoxItem>>((ob: Observer<Array<SdkComboBoxItem>>) => {
                                const listaTecniciCombo: Array<SdkComboBoxItem> = map(this.listaTecnici, (one: BaseRupInfo) => {
                                    const item: SdkComboBoxItem = {
                                        key: one.cf,
                                        value: one.nominativo
                                    };
                                    return item;
                                });
                                ob.next(listaTecniciCombo);
                                ob.complete();
                            });
                        };
                        mapping = false;
                ...
                this.sdkMessagePanelService.showInfo(this.messagesPanel, [
                    {
                        message: 'IMPORTA-GARA.LISTA-TECNICI-MODAL.INFO'
                    }
                ]);
            });
        }
    ...
    private manageExecutionProvider = (obj: any) => {
        if (obj != null && obj.close === true) {
            if (obj.data != null) {
                const cfTecnico: string = get(obj, 'data');
                this.emitOutput({ code: 'tecnico', data: cfTecnico });
            } else {
    ...
    private loadButtons(): void {
        this.buttons = {
            buttons: this.protectionUtilsService.checkButtonsProtection(this.config.listaTecniciData.buttons, this.userProfile.configurations)
        };
