...
@Component({
    templateUrl: `modifica-fase-esito-subappalto-section.widget.html`,
    encapsulation: ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.OnPush
})
export class ModificaFaseEsitoSubappaltoSectionWidget extends FaseAbstractSectionWidget {

    // #region Variables

    @HostBinding('class')
    public classNames = `modifica-fase-esito-subappalto-section`;
    ...
    protected customPopulateFunction = (field: SdkFormBuilderField, restObject?: FaseSubappaltoEntry, dynamicField?: boolean) => {
        ...
        if(field.code === 'esito'){
            if(this.fase.dataAuth != null){
                field.data = { key: '1', value: 'Positivo' };
                mapping = false;
            }

            if(this.fase.motivoMancatoSub != null){
                field.data = { key: '2', value: 'Negativo' };
                mapping = false;
            }
        }
    ...
    protected dettaglioFaseFactory(codGara: string, codLotto: string, progressivo: string): () => Observable<FaseSubappaltoEntry> {
        return () => {
            return this.faseEsitoSubappaltoService.getFase(codGara, codLotto, progressivo);
        }
    }
