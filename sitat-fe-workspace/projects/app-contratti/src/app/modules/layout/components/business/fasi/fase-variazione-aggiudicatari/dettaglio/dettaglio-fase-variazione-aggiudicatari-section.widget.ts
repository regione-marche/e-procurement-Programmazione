...
import { cloneDeep, get, isEmpty, isObject, isString, remove, set, toString } from 'lodash-es';
...
import { FaseVariazioneAggiudicatariEntry } from 'projects/app-contratti/src/app/modules/models/fasi/fase-variazione-aggiudicatari.model';
...
@Component({
    templateUrl: `dettaglio-fase-variazione-aggiudicatari-section.widget.html`,
    encapsulation: ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.OnPush
})
export class DettaglioFaseVariazioneAggiudicatariSectionWidget extends FaseAbstractSectionWidget {

    // #region Variables

    @HostBinding('class') classNames = `dettaglio-fase-variazione-aggiudicatari-section`;

    @ViewChild('messages') _messagesPanel: ElementRef;

    @ViewChild('infoBox') _infoBox: ElementRef;

    private fase: FaseVariazioneAggiudicatariEntry;
    ...
    public manageFormClick(config: SdkTextOutput): void {
        if (isObject(config)) {
            ...
            if (config.code === 'denominazione-impresa') {
