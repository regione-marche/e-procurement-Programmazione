...
import { InizFaseAggiudicazioneEntry } from 'projects/app-contratti/src/app/modules/models/fasi/fase-iniziale.model';
import { FaseAggiudicazioneService } from 'projects/app-contratti/src/app/modules/services/fasi/fase-aggiudicazione.service';
...
@Component({
    templateUrl: `nuova-fase-agg-section.widget.html`,
    encapsulation: ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.OnPush
})
export class NuovaFaseAggiudicazioneSectionWidget extends FaseAbstractSectionWidget {

    // #region Variables

    private iniz: InizFaseAggiudicazioneEntry;

    @HostBinding('class') classNames = `nuova-fase-aggiudicazione-section`;
    ...
    protected customPopulateFunction = (field: SdkFormBuilderField, restObject?: any, dynamicField?: boolean) => {
        ...
        if (isObject(this.iniz)) {
            ...
            if (field.code === 'data-verb-aggiudicazione' && this.iniz.dataVerbaleAgg != null) {               
                field.data = new Date(this.iniz.dataVerbaleAgg);
                mapping = false;
            }           
        }
    ...
    protected loadDettaglio = (): Observable<InizFaseAggiudicazioneEntry> => {
        return this.getDettaglioObservable();

    }

    private getDettaglioObservable(): Observable<InizFaseAggiudicazioneEntry> {
        let factory = this.datiInizializzazioneFaseFactory(this.codGara, this.codLotto);
        return this.requestHelper.begin(factory, this.messagesPanel)
            .pipe(
                map((result: InizFaseAggiudicazioneEntry) => {
                    this.iniz = result;
                    return result
                })
            );
    ...
    private datiInizializzazioneFaseFactory(codGara: string, codLotto: string): () => Observable<InizFaseAggiudicazioneEntry> {
        return () => {
            return this.faseAggiudicazioneService.getDatiInizializzazione(codGara, codLotto)
                .pipe(map((result: ResponseResult<InizFaseAggiudicazioneEntry>) => {
