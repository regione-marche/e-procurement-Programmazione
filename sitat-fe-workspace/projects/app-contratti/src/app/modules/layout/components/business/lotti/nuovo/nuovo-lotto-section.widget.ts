...
import { GaraEntry, LottoDynamicValue, LottoEntry, ResponseInizInsertLotto } from '../../../../../models/gare/gare.model';
import { GareService } from '../../../../../services/gare/gare.service';


@Component({
    templateUrl: `nuovo-lotto-section.widget.html`,
    encapsulation: ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.OnPush
})
export class NuovoLottoSectionWidget extends SdkBusinessAbstractWidget<void> implements OnInit, OnDestroy {

    // #region Variables

    @HostBinding('class') classNames = `nuovo-lotto-section`;
    ...
    protected onAfterViewInit(): void {
        this.loadTabellati().pipe(
            ...
            map(this.loadDynamicFactory),
            mergeMap(this.loadDynamic),
            map(this.elaborateDynamic)
        ).subscribe();
    }
    ...
    private dettaglioDynamicFactory(): () => Observable<ResponseInizInsertLotto> {
        return () => {
            return this.gareService.getInizInsertLotto();
        }
    }
    ...
    private loadDynamicFactory = (): Function => {
        let factory = this.dettaglioDynamicFactory();
        return factory;
    }

    private loadDynamic = (factory: Function): Observable<ResponseInizInsertLotto> => {
        return this.requestHelper.begin(factory, this.messagesPanel);
    }

    private elaborateDynamic = (result: ResponseInizInsertLotto) => {
        this.lotto = {
            condizioniRicorsoProcNeg: result.condizioniRicorsoProcNeg,
            modalitaAcquisizione: result.modalitaAcquisizione,
            modalitaTipologiaLavoro: result.modalitaTipologiaLavoro
        };
        this.checkInfoBox();
        this.loadForm();
    };
