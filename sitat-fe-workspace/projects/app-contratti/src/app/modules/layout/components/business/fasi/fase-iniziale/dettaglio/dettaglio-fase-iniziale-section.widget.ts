...
@Component({
    templateUrl: `dettaglio-fase-iniziale-section.widget.html`,
    encapsulation: ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.OnPush
})
export class DettaglioFaseInizialeSectionWidget extends FaseAbstractSectionWidget {

    // #region Variables

    @HostBinding('class') classNames = `dettaglio-fase-iniziale-section`;
    ...
    protected customPopulateFunction = (field: SdkFormBuilderField, restObject?: FaseInizialeEsecuzioneEntry, dynamicField?: boolean) => {
        ...
        if(!this.gara.pcp){
            if(field.code === 'codcontratto'){
                field.visible = false;
                mapping = false;
            }

            if(field.code === 'gg-data-stipula'){
                field.visible = false;
                mapping = false;
            }
            ...
            if(field.code === 'gg-data-stipula' && value == null){
                field.visible = false;
                mapping = false;
            }
        }
    ...
    protected elaborateGara = (result: GaraEntry) => {
        ...
        if(this.gara.pcp){
            ...
            this.config.menuTabs = filter(this.config.menuTabs, (one) => one.code !== 'dettaglio-scheda-sicurezza-fase-ini' && one.code !== 'dettaglio-mis-agg-sic-fase-ini' && one.code !== 'dettaglio-inc-prof-fase-ini');
