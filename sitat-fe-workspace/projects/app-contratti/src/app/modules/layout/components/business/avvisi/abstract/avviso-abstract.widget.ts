...
@Directive()
export abstract class AvvisoAbstractWidget extends SdkBusinessAbstractWidget<void> implements OnInit, AfterViewInit, OnDestroy {
    ...
    protected avviso: AvvisoEntry;
    protected tipiAvvisoMap: IDictionary<string> = {};
    ...
    protected onInit(): void {
        ...
        this.tabellatiCacheService.getValoriTabellato(Constants.TIPO_AVVISO)
            .pipe(
                ...
                mergeMap(() => this.tabellatiCacheService.getValoriTabellato(Constants.PROVINCE)),
                map((province: Array<ValoreTabellato>) => {
    ...
    protected onAfterViewInit(): void {
        this.isUpdate = isObject(this.avviso);
        ...
        if (isObject(this.config)) {
            this.markForCheck(() => {
                ...
                if (this.isUpdate && this.stazioneAppaltanteInfo != null && this.stazioneAppaltanteInfo.codice === '*') {
                    providerArgs = {
                        stazioneAppaltante: { codice: this.avviso.stazioneAppaltante }
                    }
                }
                ...
                let customPopulateFunction: CustomParamsFunction = (field: SdkFormBuilderField) => {
                    ...
                    if (this.isUpdate) {
                        if (field.type === 'COMBOBOX') {
                            let key: string = get(this.avviso, 'tipoAvviso') + '';
                            let value = this.tipiAvvisoMap[key];
                            ...
                            mapping = false;
                        } else if (field.type === 'DATEPICKER' && !isEmpty(get(this.avviso, field.mappingInput))) {
                            set(field, 'data', new Date(get(this.avviso, field.mappingInput)));
                            mapping = false;
                        } else if (field.code === 'comune' && field.type === 'AUTOCOMPLETE') {
                            let data: string = get(this.avviso, field.mappingInput);
                            if (data != null) {
                            ...
                            let data: Tecnico = get(this.avviso, 'rupEntry');
                            if (data) {
                                let item: SdkAutocompleteItem = {
                                    ...data,
                            ...
                            let data: UffEntry = get(this.avviso, 'ufficio');
                            if (data) {
                                let item: SdkAutocompleteItem = {
                                    ...data,
                ...
                formConfig = this.formBuilderUtilsService.populateForm(formConfig, this.isUpdate, customPopulateFunction, this.avviso, providerArgs);
    ...
    public manageOutputField(field: SdkFormBuilderField): void {
        if (field != null && field.code === 'comune') {
    ...
    public onButtonClick(button: SdkButtonGroupOutput): void {
        if (isObject(button) && isEmpty(button.provider) === false) {
            ...
            let stazioneAppaltante = this.avviso == null ? this.stazioneAppaltanteInfo.codice : this.avviso.stazioneAppaltante;
