...
import { cloneDeep, find, get, isEmpty, isString, toString } from 'lodash-es';
...
@Component({
    templateUrl: `nuova-fase-superamento-quarto-contrattuale-section.widget.html`,
    encapsulation: ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.OnPush
})
export class NuovaFaseSuperamentoQuartoContrattualeSectionWidget extends FaseAbstractSectionWidget {

    // #region Variables

    @HostBinding('class') classNames = `nuova-fase-superamento-quarto-contrattuale-section`;
            ...
            if (field.data != null) {
                this.fase.listaSosp.forEach((fase: FaseSospensioneEntry) => {
                    if(fase.num.toString() === field.data.key){
                        ...
                        if(motivoSospValue != null && motivoSospValue.length > 0){
                            this.formBuilderDataSubject.next({
                                code: 'motivo-sosp',
    ...
    private datiInizializzazioneFaseFactory(codGara: string, codLotto: string): () => Observable<InizFaseRipresaPrestazioneEntry> {
        return () => {
            return this.faseSuperamentoQuartoContrattualeService.getDatiInizializzazione(codGara, codLotto);
        }
    }
