...
export class FormBuilderUtilsService extends SdkBaseService {
    ...
    public populateListCode(valoriTabellati: IDictionary<Array<ValoreTabellato>>, field: SdkFormBuilderField, restValue: any, mapping: boolean, annoInizio?: number): Array<any> {
        ...
        if (comboCode === 'sino') {
            set(field, 'data', restValue.trim() === '2' ? this.translateService.instant('COMBOBOX.NO') : this.translateService.instant('COMBOBOX.SI'));
            mapping = false;
        }if (comboCode === 'sinoboolean') {
            set(field, 'data', restValue.trim() === '0' ? this.translateService.instant('COMBOBOX.NO') : this.translateService.instant('COMBOBOX.SI'));
            mapping = false;
        } else {
            let tabellati: Array<ValoreTabellato> = get(valoriTabellati, comboCode);
            each(tabellati, (tabellato: ValoreTabellato) => {
                if (tabellato.codice.trim() === restValue.trim()) {
