...
export interface AdvancedSearchArchivioCdcProviderArgs extends IDictionary<any> {
...
@Injectable()
export class AdvancedSearchArchivioCdcProvider extends SdkBaseService implements SdkProvider {

    constructor(inj: Injector) {
        super(inj);
    }

    public run(args: AdvancedSearchArchivioCdcProviderArgs): Observable<IDictionary<any>> {
        this.logger.debug('AdvancedSearchArchivioCdcProvider', args);
        ...
        if (args.buttonCode === 'back-button') {
            ...
            this.store.dispatch(new SdkStoreAction(Constants.SEARCH_FORM_ARCHIVIO_CDC_DISPATCHER, form));
            this.routerService.navigateToPage('lista-archivio-cdc-page');
        }
        return new Observable((ob: Observer<IDictionary<any>>) => {
            ob.next(args);
            ob.complete();
