...
import { ListaArchivioTecniciDatasource } from './lista-archivio-tecnici.datasource.service';

@Component({
    templateUrl: `lista-archivio-tecnici-section.widget.html`,
    encapsulation: ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.OnPush
})
export class ListaArchivioTecniciSectionWidget extends SdkBusinessAbstractWidget<void> implements OnInit, OnDestroy {

    // #region Variables

    @HostBinding('class') classNames = `lista-archivio-tecnici-section`;
    ...
    private searchForm: RicercaAvanzataArchivioTecniciForm;
    private stazioneAppaltante: string;
    private buttons: SdkButtonGroupInput;
    private userProfile: UserProfile;
    private gridConfig: SdkTableConfig;
    private datasource: ListaArchivioTecniciDatasource;
    ...
    private initPerformers(): void {
        this.performers = {
            dettaglio: (selectedRow: { rowIndex: number, dataItem: SdkTableRowDto }) => {
                ...
                let item: RupEntry = tempItem as RupEntry;
                this.detailTecnico(toString(item.codice));
            },
            delete: (selectedRow: { rowIndex: number, dataItem: SdkTableRowDto }) => {
                let tempItem: unknown = selectedRow.dataItem as unknown;
                let item: RupEntry = tempItem as RupEntry;
                this.deleteTecnico(toString(item.codice));
            },
    ...
    public detailTecnico(codice: string): void {
        this.unsubscribeQuerySearchSubscription();
        this.provider.run('APP_COMMONS_LISTA_TECNICI', {
            action: 'DETAIL',
            codice,
            messagesPanel: this.messagesPanel
        }).subscribe();
    ...
    private deleteTecnico(codice: string): void {
        let func = this.deleteTecnicoConfirm(codice);
        this.dialogConfig.open.next(func);
    }

    private deleteTecnicoConfirm(codice: string): any {
        return () => {
    ...
    private initGrid(): void {
        ...
        this.datasource = this.factory.create(ListaArchivioTecniciDatasource, {
