...
import { AuthenticationTokenInfo, RefreshTokenRequest } from '../../models/authentication/authentication.model';
...
export class HttpRequestHelper extends SdkBaseService {
    ...
    private _unsuccessful = (ob: Observer<any>, error: HttpErrorResponse, errorPanel: HTMLElement, loading: boolean, scrollView: boolean) => {
        ...
        setTimeout(() => {
            ...
            if (!isEmpty(errorDataParameters)) {
                let translatedData: string = this.translateService.instant(message, errorDataParameters);
    ...
    private _unsuccessfulRefresh = (ob: Observer<any>, error: Error, message: any, loading: boolean) => {
        ...
        setTimeout(() => {
            ...
            let redirectUrl: string = this.appConfig.environment.APP_LAUNCHER_CONTEXT_URL;
    ...
    private _refreshToken = (serviceFunction: Function, ob: Observer<any>, loading: boolean, scrollView: boolean, errorPanel?: HTMLElement) => {
        setTimeout(() => {
            ...
            if (this.appConfig.environment.LOGIN_MODE === 0) {
                ...
                let appCode: string = this.sdkSessionStorageService.getItem<string>(Constants.AUTHENTICATION_APP_CODE, Constants.LOCAL_STORAGE_PREFIX);
                let authTokenInfo: AuthenticationTokenInfo = this.sdkSessionStorageService.getItem<AuthenticationTokenInfo>(Constants.AUTHENTICATION_TOKEN_INFO, Constants.LOCAL_STORAGE_PREFIX);
                ...
                this.restHelper.post<AuthenticationTokenInfo>(`${this.appConfig.environment.AUTHENTICATION_BASE_URL}/refreshToken`, body).subscribe(
                    (response: any) => {
                        ...
                        this.sdkSessionStorageService.setItem(Constants.AUTHENTICATION_TOKEN_INFO, authData, Constants.LOCAL_STORAGE_PREFIX);
