...
export class FormBuilderUtilsService extends SdkBaseService {
    ...
    private getKeyValueAt(config: any, restObject: any, keyNum: number, defaultVal: string = '#') {
        if(config.entity.keys.length >= keyNum) {
            let keyField = config.entity.keys[keyNum - 1];
            let keyValue = get(restObject, keyField);
            if(keyValue == null) {
                throw new Error("Valore nullo per chiave:" + keyField +  " - entity:" + config.entity.name);
            }
            return keyValue;
        }
        return defaultVal;
    ...
    private populatesC0oggassParams(config: any, restObject: any) {
        if (config != null && config.entity != null && !isEmpty(config.entity.name) && isArray(config.entity.keys)) {
            this.sdkC0oggassParamsStoreService.clear();
            if (restObject != null) {
                this.sdkC0oggassParamsStoreService.parentBreadcrumbs = config.breadcrumbs;
                this.sdkC0oggassParamsStoreService.c0oggassParams = {
                    c0aprg: 'W9',
                    c0aent: config.entity.name,
                    c0akey1: this.getKeyValueAt(config, restObject, 1),
                    c0akey2: this.getKeyValueAt(config, restObject, 2),
                    c0akey3: this.getKeyValueAt(config, restObject, 3),
                    c0akey4: this.getKeyValueAt(config, restObject, 4),
                    c0akey5: this.getKeyValueAt(config, restObject, 5),
                };
            }
        }
    }
     ...
     */
    public populateForm(formConfig: SdkFormBuilderConfiguration, populate: boolean = false, customParamsFunction?: CustomParamsFunction, restObject?: any, providerArgs?: IDictionary<any>, infoBox?: boolean,config?: any,): SdkFormBuilderConfiguration {

        if(config?.pageFeatures?.includes('docassociati')){ this.populatesC0oggassParams(config, restObject) }
    ...
    public populateListCode(valoriTabellati: IDictionary<Array<ValoreTabellato>>, field: SdkFormBuilderField, restValue: any, mapping: boolean, annoInizio?: number): Array<any> {
        ...
        if (comboCode === 'sino') {
            ...
            mapping = false;
        } else if (comboCode === 'EsitoSubappalto') {
            set(field, 'data', restValue === '2' ? this.translateService.instant('COMBOBOX.NEGATIVO') : this.translateService.instant('COMBOBOX.POSITIVO'));
            mapping = false;
        } else if (comboCode === 'EsecuzioneSuba') {
            set(field, 'data', restValue === '2' ? this.translateService.instant('COMBOBOX.NON-ESEGUITO') : this.translateService.instant('COMBOBOX.ESEGUITO'));
            mapping = false;
        } else if (comboCode === 'tipoImpresa') {
            set(field, 'data', restValue === '2' ? this.translateService.instant('COMBOBOX.MANDATARIA') : this.translateService.instant('COMBOBOX.IMPRESA-SINGOLA'));
            mapping = false;
        } else if (comboCode === 'annoriferimento') {
            if (restValue === '1') {
                set(field, 'data', annoInizio);
                mapping = false;
            } else if (restValue === '2') {
                set(field, 'data', annoInizio + 1);
                mapping = false;
            } else if (restValue === '3') {
                set(field, 'data', annoInizio + 2);
                mapping = false;
            }
        } else if (comboCode === 'settorecustom') {
            let tabellati: Array<ValoreTabellato> = get(valoriTabellati, 'TipoAppalto');
            let filteredArr = filter(tabellati, (one: ValoreTabellato) => one.codice !== 'L');
            each(filteredArr, (tabellato: ValoreTabellato) => {
                if (tabellato.codice === restValue) {
                    set(field, 'data', tabellato.descrizione);
                    mapping = false;
                }
            });
        } else if (comboCode === 'CriterioAggiudicazioneSimog1') {
            let tabellati: Array<ValoreTabellato> = get(valoriTabellati, 'CriterioAggiudicazione');
            let filteredArr = filter(tabellati, (one: ValoreTabellato) => toNumber(one.codice) <= 2);
            each(filteredArr, (tabellato: ValoreTabellato) => {
                if (tabellato.codice === restValue) {
                    set(field, 'data', tabellato.descrizione);
                    mapping = false;
        ...
        } else if (comboCode === 'TipologiaSoggettoAggiudicazione') {
            let tabellati: Array<ValoreTabellato> = get(valoriTabellati, 'TipologiaSoggetto');
            let codici: Array<string> = ['1', '2', '3', '4', '5', '6', '7', '8', '14', '15', '16', '17', '18', '19'];
            let filteredArr = filter(tabellati, (one: ValoreTabellato) => includes(codici, one.codice));
            each(filteredArr, (tabellato: ValoreTabellato) => {
                if (tabellato.codice === restValue) {
                    set(field, 'data', tabellato.descrizione);
                    mapping = false;
        ...
        } else if (comboCode === 'TipologiaSoggettoPcp') {
            let tabellati: Array<ValoreTabellato> = get(valoriTabellati, 'TipologiaSoggetto');
        ...
        } else if (comboCode === 'TipologiaSoggettoAggiudicazioneSemplificata') {
            let tabellati: Array<ValoreTabellato> = get(valoriTabellati, 'TipologiaSoggetto');
        ...
        } else if (comboCode === 'TipologiaSoggettoIniziale') {
            let tabellati: Array<ValoreTabellato> = get(valoriTabellati, 'TipologiaSoggetto');
        ...
        } else if (comboCode === 'TipologiaSoggettoAdesione') {
            let tabellati: Array<ValoreTabellato> = get(valoriTabellati, 'TipologiaSoggetto');
        ...
        } else if (comboCode === 'TipologiaSoggettoCollaudo') {
            const tabellati: Array<ValoreTabellato> = get(valoriTabellati, 'TipologiaSoggetto');
            const codici: Array<string> = ['10', '11', '12', '13'];
            const filteredArr = filter(tabellati, (one: ValoreTabellato) => includes(codici, one.codice));
            each(filteredArr, (tabellato: ValoreTabellato) => {
                if (tabellato.codice === restValue) {
                    set(field, 'data', tabellato.descrizione);
                    mapping = false;
        ...
        } else if (comboCode === 'TipologiaSoggettoCantieri') {
            const tabellati: Array<ValoreTabellato> = get(valoriTabellati, 'TipologiaSoggetto');
        ...
        } else if (comboCode === 'TipologiaSoggettoInizialeSemp') {
            let tabellati: Array<ValoreTabellato> = get(valoriTabellati, 'TipologiaSoggetto');
        ...
        } else if (comboCode === 'MotivoInterruzioneSimogEq1') {
            let tabellati: Array<ValoreTabellato> = get(valoriTabellati, 'MotivoInterruzione');
            let filteredArr = filter(tabellati, (one: ValoreTabellato) => +one.codice <= 5);
            each(filteredArr, (tabellato: ValoreTabellato) => {
                if (tabellato.codice === restValue) {
                    set(field, 'data', tabellato.descrizione);
                    mapping = false;
        ...
        } else if (comboCode === 'MotivoRisoluzioneSimoqEq1') {
            let tabellati: Array<ValoreTabellato> = get(valoriTabellati, 'MotivoRisoluzione');
            let filteredArr = filter(tabellati, (one: ValoreTabellato) => +one.codice <= 5);
            each(filteredArr, (tabellato: ValoreTabellato) => {
                if (tabellato.codice === restValue) {
                    set(field, 'data', tabellato.descrizione);
        ...
        } else if (comboCode === 'TipoRealizzazioneSimog1') {
            let tabellati: Array<ValoreTabellato> = get(valoriTabellati, 'TipoRealizzazione');
            let filteredArr = filter(tabellati, (one: ValoreTabellato) => +one.codice <= 12);
            each(filteredArr, (tabellato: ValoreTabellato) => {
                if (tabellato.codice === restValue) {
                    set(field, 'data', tabellato.descrizione);
                    mapping = false;
    ...
    public elaborateFieldValidators(field: SdkFormBuilderField): SdkFormBuilderField {
        if (field != null && !isEmpty(field.validators)) {
            each(field.validators, (one: any) => {
                ...
                } else if(isString(one.validator)){
