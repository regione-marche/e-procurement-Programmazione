...
@Component({
  templateUrl: './sdk-dettaglio-server-posta.component.html'
})
export class SdkDettaglioServerPostaComponent extends SdkBusinessAbstractWidget<void> implements OnInit, OnDestroy {

  // #region Variables

  @HostBinding('class') classNames = `sdk-dettaglio-server-posta-section`;
  ...
  protected onAfterViewInit(): void {
    this.loadDettaglio().pipe(
      ...
      map(() => {
        if (this.changed != null && this.changed === 'Y') {
          this.sdkMessagePanelService.showSuccess(this.messagesPanel, [
            {
              message: 'SDK-SERVER-POSTA.CHANGE-PASSWORD-DONE'
            }
          ]);
        } else if (this.sent != null && this.sent === 'Y') {
          this.sdkMessagePanelService.showSuccess(this.messagesPanel, [
            {
              message: 'SDK-SERVER-POSTA.TEST-SEND-DONE'
            }
          ]);
        }
      })
    ...
    if (this.config != null) {
      this.markForCheck(() => {
        ...
        let form: IDictionary<any> = {
          ...this.dettaglio,
          porta: this.dettaglio.porta === '25' ? `${this.dettaglio.porta} (default)` : this.dettaglio.porta,
  ...
  private loadQueryParams(): void {
    ...
    this.changed = paramsMap.get('changed');
    this.sent = paramsMap.get('sent');
  }
