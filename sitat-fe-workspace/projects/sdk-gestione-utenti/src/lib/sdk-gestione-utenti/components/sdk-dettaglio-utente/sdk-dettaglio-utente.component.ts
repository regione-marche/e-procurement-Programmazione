...
import { cloneDeep, get, isEmpty, map as mapArray, remove, toString } from 'lodash-es';
...
@Component({
  templateUrl: './sdk-dettaglio-utente.component.html'
})
export class SdkDettaglioUtenteComponent extends SdkBusinessAbstractWidget<void> implements OnInit, OnDestroy {

  // #region Variables

  @HostBinding('class') classNames = `sdk-dettaglio-utente-section`;
    ...
    if (this.config != null) {
      this.markForCheck(() => {
        ...
        let form: IDictionary<any> = {
          ...
          isAmministratore: this.isAmministratore(),
        ...
        let customPopulateFunction: CustomParamsFunction = (field: SdkFormBuilderField, restObject: any, dynamicField: boolean) => {
          ...
          if (field.code === 'data-scadenza-account') {
  ...
  private loadQueryParams(): void {
    ...
    this.warning = paramsMap.get('warning');
  }
  ...
  private loadWarning(): void {
    if (this.warning != null) {
      if (this.warning == 'GESTIONE_UTENTE_USER_EDIT_AMMINISTRAZIONI_WITHOUT_GESTIONE_COMPLETA') {
        this.sdkMessagePanelService.showWarning(this.messagesPanel, [
          {
            message: 'WARNINGS.GESTIONE_UTENTE_USER_EDIT_AMMINISTRAZIONI_WITHOUT_GESTIONE_COMPLETA'
          }
        ]);
      }
      if (this.warning == 'GESTIONE_UTENTE_USER_DELEGATO_ALL_SA') {
        this.sdkMessagePanelService.showWarning(this.messagesPanel, [
          {
            message: 'WARNINGS.GESTIONE_UTENTE_USER_DELEGATO_ALL_SA'
          }
        ]);
      }
    }
