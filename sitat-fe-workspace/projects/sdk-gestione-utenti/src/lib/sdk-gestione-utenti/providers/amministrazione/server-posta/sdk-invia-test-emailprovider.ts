...
import { WMailDTO, WMailTestSendDTO } from '../../../model/amministrazione.model';
...
export interface SdkInviaTestEmailPostaProviderArgs extends IDictionary<any> {
...
@Injectable()
export class SdkInviaTestEmailPostaProvider extends SdkBaseService implements SdkProvider {

    constructor(inj: Injector) {
        super(inj);
    }

    public run(args: SdkInviaTestEmailPostaProviderArgs): Observable<IDictionary<any>> {
        this.logger.debug('SdkInviaTestEmailPostaProviderArgs >>>', args);

        if (args.buttonCode === 'invia-test-email') {
            ...
            if (!isEqual(defaultFormBuilderConfig, formBuilderConfig)) {
                ...
                if (valid === true) {
                    ...
                    let form: WMailTestSendDTO = this.populateRequest(formBuilderConfig, item);

                    return this.gestioneServerPostaService.invioTestEmail(form).pipe(
                        map((result: ResponseDTO<boolean>) => {
                            ...
                            if (result.done === SdkGestioneUtentiConstants.RESPONSE_N && result.messages != null && result.messages.length > 0) {
                                ...
                                this.sdkDettaglioServerPostaStoreService.idCfg = item.idCfg;
    ...
    private populateRequest(formBuilderConfig: SdkFormBuilderConfiguration, item: WMailDTO): WMailTestSendDTO {
        let request: WMailTestSendDTO = this.getDefaultForm(item);
    ...
    private getDefaultForm(item: WMailDTO): WMailTestSendDTO {
        return {
            idCfg: item.idCfg,
            testEmail: null
        };
    ...
    private elaborateOne(field: SdkFormBuilderField, request: WMailTestSendDTO): WMailTestSendDTO {
    ...
    private elaborateSection(field: SdkFormBuilderField, request: WMailTestSendDTO): WMailTestSendDTO {
    ...
    private elaborateGroup(field: SdkFormBuilderField, request: WMailTestSendDTO): WMailTestSendDTO {
