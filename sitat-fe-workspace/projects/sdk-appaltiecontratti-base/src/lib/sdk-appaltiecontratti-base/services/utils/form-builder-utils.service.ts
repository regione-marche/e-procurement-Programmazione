...
import { ComboDto, DynamicValue } from '../../model/sdk-base.model';
...
export class FormBuilderUtilsService extends SdkBaseService {
    ...
    private readonly ABILITAZIONE_AMMINISTRATORE: string = 'ou89';
     ...
     */
    public populateForm(config: any, formConfig: SdkFormBuilderConfiguration, populate: boolean = false, customParamsFunction?: CustomParamsFunction, restObject?: any, providerArgs?: IDictionary<any>, infoBox?: boolean): SdkFormBuilderConfiguration {
    ...
    public isAmministratore(abilitazioni: Array<string>): boolean {
        if (isArray(abilitazioni) && !isEmpty(abilitazioni)) {
            return indexOf(abilitazioni, this.ABILITAZIONE_AMMINISTRATORE) > -1
        }
        return false;
    }
        ...
        if (config != null && config.entity != null && !isEmpty(config.entity.name) && isArray(config.entity.keys)) {
            ...
            if (restObject != null) {
                ...
                this.sdkC0oggassParamsStoreService.c0oggassParams = {
                    c0aprg: this.appConfig.environment.APP_CODE,
    ...
    public populateListCode(valoriTabellati: IDictionary<Array<ComboDto>>, field: SdkFormBuilderField, restValue: any, mapping: boolean, annoInizio?: number): Array<any> {
        let comboCode: string = field.listCode;
        let showKeyInDescription: boolean = !!field.showKeyInDescription;
        if (comboCode === 'sino') {
            set(field, 'data', restValue === true || restValue === 'true' || restValue === '1' ? this.translateService.instant('COMBOBOX.SI') : this.translateService.instant('COMBOBOX.NO'));
            mapping = false;
            ...
            let tabellati: Array<ComboDto> = get(valoriTabellati, comboCode);
            each(tabellati, (tabellato: ComboDto) => {
                if (tabellato.key === restValue) {
                    set(field, 'data', tabellato.value);
                    mapping = false;
                }
            });
        }
        if (mapping === false && showKeyInDescription === true) {
            field.data = `${restValue} - ${field.data}`;
        }
    ...
    protected elaborateOne(
        ...
        if (populate === true && mapping === true && dynamicField === false) {
            ...
            } else if (field.type === 'TEXT' && field.date === true) {
                let value: string = get(restObject, field.mappingInput);
                if (value != null) {
                    let formatted = this.dateHelper.format(new Date(value), this.localeService.dateFormat);
        ...
        if (field.type === 'AUTOCOMPLETE' && field.advanced === true && field.advancedModalComponentConfig != null) {
            field.advancedModalComponentConfig.itemsProvider = this.provider.run(field.advancedModalComponentConfig.itemsProviderCode, providerArgs);
        }
    ...
    protected get localeService(): SdkLocaleService { return this.injectable(SdkLocaleService) }
