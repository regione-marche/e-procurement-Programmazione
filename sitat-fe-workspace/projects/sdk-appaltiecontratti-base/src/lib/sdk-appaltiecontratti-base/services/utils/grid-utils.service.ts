...
import { ProfiloConfiguration } from '../../model/profilo/profilo.model';
        ...
        each(grid.columns, (one: SdkTableColumn) => {
            if (isObject(one.viewer)) {
                ...
                if (type === 'SDK_GRID_VIEW_CELL_DATE') {
                    if(!one.viewer.params) {
                        set(one.viewer, 'params', {});
                    }
                    if(isEmpty(get(one.viewer.params, 'format'))) {
                        set(one.viewer.params, 'format', format);
                    }
                }
                if (type === 'SDK_GRID_VIEW_CELL_ACTIONS' || type === 'SDK_GRID_VIEW_CELL_ACTIONS_ICON') {
