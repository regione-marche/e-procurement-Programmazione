...
import it.appaltiecontratti.pubblprogrammi.entity.DettaglioFornitureServiziResult;
import it.appaltiecontratti.pubblprogrammi.entity.DettaglioLavoriResult;
...
import it.appaltiecontratti.pubblprogrammi.entity.TabellatoEntry;
...
import it.appaltiecontratti.pubblprogrammi.mapper.TabellatiMapper;
...
public class ValidateManager {
	...
	public void validatePubblicaProgrammaLavori(PubblicaProgrammaLavoriEntry programma, List<ValidateEntry> controlli)
		...
		if (programma.getIdRicevuto() != null) {
			...
			if (i == 0) {
				ValidateEntry item = new ValidateEntry("idRicevuto",
						"L'id programma indicato non esiste nella banca dati");
				controlli.add(item);
			} else {
				// se esiste verifico che idclient sia uguale
				i = sqlMapper.count("PIATRI WHERE ID_GENERATO=" + programma.getIdRicevuto() + " and ID_CLIENT='"
						+ programma.getClientId() + "'");
				if (i == 0) {
					ValidateEntry item = new ValidateEntry("idRicevuto",
							"L'id programma indicato esiste già per un'altra utenza");
					controlli.add(item);
				}
			}
		}
		...
		if (programma.getId() != null && programma.getId().length() > 0) {
			...
			// verifico se esiste già un programma per l'amministrazione dello stesso anno e
			// tipo
			...
			if (i > 0) {
				ValidateEntry item = new ValidateEntry("id", "Il programma con id " + programma.getId()
						+ "esiste già nella banca dati e non è possibile proseguire con l'aggiornamento");
				controlli.add(item);
			}
		} else {
	...
	private void validateInterventoLavori(InterventoEntry intervento, List<ValidateEntry> controlli)
			throws IOException {
	...
	private void validateImmobile(ImmobileEntry immobile, InterventoEntry intervento, List<ValidateEntry> controlli)
			throws IOException {
	...
	private void validateInterventoNonRiproposto(InterventoNonRipropostoEntry interventoNonRiproposto,
	...
	private void validateAcquistoNonRiproposto(AcquistoNonRipropostoEntry acquistoNonRiproposto,
			List<ValidateEntry> controlli) throws IOException {
	...
	private void validateAltriDatiOperaIncompiuta(AltriDatiOperaIncompiutaEntry altriDati,
			List<ValidateEntry> controlli) throws IOException {
	...
	private void validateImmobileOpera(ImmobileEntry immobile, OperaIncompiutaEntry opera, String riferimento,
			List<ValidateEntry> controlli) throws IOException {
	...
	private void validatePubblicaTecnico(DatiGeneraliTecnicoEntry tecnico, String riferimento,
			List<ValidateEntry> controlli) throws IOException {
		...
		if (codFunzione.equals("PUBBLICA_FS")) {
			...
			for (ControlloEntry controllo : controlliDaFare) {
				...
				case 180:
					// Tipologia apporto di capitale privato
					if (intervento.getTipologiaCapitalePrivato() == null
							&& intervento.getImportoCapitalePrivato() > 0) {
						condizioneVerificata = true;
					}
					break;
				case 190:
