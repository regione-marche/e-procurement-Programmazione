package it.appaltiecontratti.monitoraggiocontratti.contratti.bl.v1021;
...
import io.swagger.client.api.v1021.AusaResourceV1Api;
import io.swagger.client.api.v1021.CodeListResourceV1Api;
import io.swagger.client.api.v1021.ComunicaAppaltoResourceV1Api;
import io.swagger.client.api.v1021.ComunicaPostPubblicazioneResourceV1Api;
import io.swagger.client.api.v1021.GestioneUtentiResourceV1Api;
import io.swagger.client.api.v1021.ServiziComuniResourceV1Api;
...
import it.appaltiecontratti.pcp.v1021.comunicaAppalto.CigType;
import it.appaltiecontratti.pcp.v1021.comunicaAppalto.ConsultaAppaltoResponse;
import it.appaltiecontratti.pcp.v1021.comunicaPostPubblicazione.CollaudoType.EsitoEnum;
...
import it.appaltiecontratti.pcp.v1021.serviziComuni.AckResponse;
import it.appaltiecontratti.pcp.v1021.serviziComuni.EsitoOperazioneListaResponse;
import it.appaltiecontratti.pcp.v1021.serviziComuni.EsitoOperazioneType;
...
@Component(value = "schedePcpManagerV1021")
public class SchedePcpManagerV1021 extends AbstractManager {

	/** Logger di classe. */
	private static final Logger logger = LogManager.getLogger(SchedePcpManagerV1021.class);
	...
	private void initApi() throws Exception {
		try {
			...
			// Verifica se la stringa non termina con '/npa-gateway-ms/' oppure con una variante come '/npa-gateway-ms-qualcosa/'
			// Se la condizione è vera, aggiunge '/npa-gateway-ms/' alla fine della stringa
			//if (!anacGatewayPath.matches(".*/npa-gateway-ms(/|$)") && !anacGatewayPath.matches(".*/npa-gateway-ms-[^/]+/$")) {
				 //Se la stringa termina con '/', aggiunge 'npa-gateway-ms/' altrimenti aggiunge '/npa-gateway-ms/'
		...
		try {
			...
			if(gara.getDrp() != null && gara.getCodiceTecnico() != null && gara.getDrp().equals(gara.getCodiceTecnico())) {
				...
				if (gara.getDrp() != null) {
					...
					if(drp!= null && cf.equals(drp.getCf())){
						xUserRole = "DRP3";
					}
				} else {
					...
					if (gara.getCodiceTecnico() != null) {
						...
						if(tecnico != null && cf.equals(tecnico.getCf())){
							xUserRole = "RP";
						}
					}
				}
...
//					reinvio = invio2 - invio1 == 0L ? true : false;
					...
					break;
				case 21:
					SchedaS4Type schedas4Type = new SchedaS4Type();
					xTipologicaScheda = FasiPcp.S4;
					FaseVariazioneAggiudicatariEntry va = this.contrattiMapper.getFaseVariazioneAggiudicatari(codGara, codLotto, num);
					ImpresaEntry imp = null;
					if (va.getCodImpresa() != null) {
						imp = this.fasiManager.getImpresa(va.getCodImpresa());
					}
					AnacFormS4Type afS4;
					afS4 = this.variazioneAggiudicatariToType(va, lotto, imp);
					schedas4Type.setAnacForm(afS4);

					schedaJson = objectMapper.writeValueAsString(schedas4Type);
					if(!pubblicata) {
						risultato = creaScheda( EModalitaInvio.INVIA, xTipologicaScheda, schedas4Type,idAppalto, syscon, codProfilo, xUserIdpType, xUserLoa, xSAcodiceAUSA, xUserCodiceFiscale, xSACodiceFiscale, xUserRole);
						motivazione = CREA_SCHEDA;
					} else {
						risultato = confermaScheda( xTipologicaScheda, idSchedaSchedaPubblicata, idAppalto, syscon, codProfilo, xUserIdpType, xUserLoa, xSAcodiceAUSA, xUserCodiceFiscale, xSACodiceFiscale, xUserRole);
						motivazione = CONFERMA_SCHEDA;
	...
	private AnacFormS4Type variazioneAggiudicatariToType(FaseVariazioneAggiudicatariEntry va, LottoEntry lotto, ImpresaEntry imp){
		AnacFormS4Type s4 = new AnacFormS4Type();
		s4.setCig(lotto.getCig());

		if(va.getMotivoVariazione() != null){
			MotiviVariazioneAnagraficaEnum motivoVar = new MotiviVariazioneAnagraficaEnum();
			motivoVar.setCodice(va.getMotivoVariazione().toString());
			motivoVar.setIdTipologica("motivoVariazione");
			s4.setMotivoVariazione(motivoVar);
		}

		PartecipanteType part = new PartecipanteType();

		if(imp != null){
			if(imp.getCodiceFiscale() != null){
				part.setCodiceFiscale(imp.getCodiceFiscale());
			}

			if(imp.getRagioneSociale() != null){
				part.setDenominazione(imp.getRagioneSociale());
			}

			/*Nel tracciato json, “avvalimento” vale false se FLAG_AVVALIMENTO=0, true altrimenti.
			“tipologiaAvvalimento” vale FLAG_AVVALIMENTO, se >0*/

			if(imp.getNazione() != null){
				part.setPaeseOperatoreEconomico(imp.getNazione().toString());
			}
		}

		if(va.getIdPartecipante() != null){
			part.setIdPartecipante(UUID.fromString(va.getIdPartecipante()));
		}

		if(va.getFlagAvvalimento() != null && va.getFlagAvvalimento() == 0L){
			part.setAvvalimento(false);
		} else{
			part.setAvvalimento(true);
			TipologiaAvvalimentoEnum avv = new TipologiaAvvalimentoEnum();
			avv.setCodice(va.getFlagAvvalimento().toString());
			avv.setIdTipologica("tipologiaAvvalimento");
			part.setTipologiaAvvalimento(avv);
		}

		if(va.getIdRuolo() != null){
			RuoloOEEnum ruolo = new RuoloOEEnum();
			ruolo.setCodice(va.getIdRuolo().toString());
			ruolo.setIdTipologica("ruoloOE");
			part.setRuoloOE(ruolo);
		}

		if(va.getTipoOe() != null){
			TipoOEEnum tipo = new TipoOEEnum();
			tipo.setCodice(va.getTipoOe().toString());
			tipo.setIdTipologica("tipoOE");
			part.setTipoOE(tipo);
		}
		s4.setPartecipante(part);
...
//				}
	...
	private CollaudoType collaudoTypeToType(FaseCollaudoEntry cl) {
		...
		if(cl.getImportoEventualeDefAmm() != null) {
			dType.setImpTotaleDefinizione(cl.getImportoEventualeDefAmm());
		}
	...
	private ResponsePubblicaFase creaScheda(EModalitaInvio xModalitaInvio,String xTipologicaScheda,Object scheda,String idAppalto, Long syscon, String codProfilo, UserIdpTypeEnum xUserIdpType, CustomUserLoaEnum xUserLoa, String xSAcodiceAUSA, String xUserCodiceFiscale, String xSACodiceFiscale, String xUserRole) throws Exception {
		...
		try {			
	        ...
	        try {
	        	res = comunicaPostPubblicazioneResourceV1Api.v1ProtectedComunicaPostPubblicazioneCreaSchedaPost(xSAcodiceAUSA, xTenantId, xUserCodiceFiscale, xUserIdpType, xUserLoa, xUserRole, body, xSACodiceFiscale, xModalitaInvio,xRegCodiceComponente, xRegCodicePiattaforma,xTipologicaScheda, xVersioneTracciato);
		...
		try {
			...
			try {
				res = comunicaPostPubblicazioneResourceV1Api
						.v1ProtectedComunicaPostPubblicazioneModificaSchedaPost(xSAcodiceAUSA, xTenantId, xUserCodiceFiscale, xUserIdpType, xUserLoa, xUserRole, body, xSACodiceFiscale, xModalitaInvio,xRegCodiceComponente, xRegCodicePiattaforma, xTipologicaScheda, xVersioneTracciato);
	...
	private ResponsePubblicaFase confermaScheda(String xTipologicaScheda,String idScheda,String idAppalto, Long syscon, String codProfilo, UserIdpTypeEnum xUserIdpType, CustomUserLoaEnum xUserLoa, String xSAcodiceAUSA, String xUserCodiceFiscale, String xSACodiceFiscale, String xUserRole) throws Exception {
		...
		try {	
			...
			InlineResponse400 res = null;
			try {																												
				res = comunicaPostPubblicazioneResourceV1Api.v1ProtectedComunicaPostPubblicazioneConfermaSchedaPost(xSAcodiceAUSA, xTenantId, xUserCodiceFiscale, xUserIdpType, xUserLoa, xUserRole, body, xSACodiceFiscale,null,xRegCodiceComponente, xRegCodicePiattaforma,xTipologicaScheda, xVersioneTracciato);
	...
	private GenericResponse<AggiungiSoggettoResponse> aggiungiSoggetto(EModalitaInvio xModalitaInvio, String xTipologicaScheda, String idAppalto, Long syscon, SoggettoRequestDTO body
		...
		try {			
			...
			try {
				res = gestioneUtentiResourceV1Api.v1ProtectedGestioneUtentiAggiungiSoggettoPost(xSAcodiceAUSA, xTenantId, xUserCodiceFiscale, xUserIdpType, xUserLoa, xUserRole, body, xSACodiceFiscale, xModalitaInvio, xRegCodiceComponente, xRegCodicePiattaforma, xTipologicaScheda, xVersioneTracciato);
	...
	private GenericResponse<EliminaSoggettoResponse> eliminaSoggetto(EModalitaInvio xModalitaInvio, String xTipologicaScheda, String idAppalto, Long syscon, EliminaSoggettoRequestDTO body
		...
		try {			
			...
			try {
				res = gestioneUtentiResourceV1Api.v1ProtectedGestioneUtentiEliminaSoggettoPost(xSAcodiceAUSA, xTenantId, xUserCodiceFiscale, xUserIdpType,  xUserLoa, xUserRole,  body, xSACodiceFiscale, xModalitaInvio, xRegCodiceComponente, xRegCodicePiattaforma, xTipologicaScheda, xVersioneTracciato);
...
//				r.setIdAppalto((String) body.get("idAppalto"));
	...
	private GenericResponse<CancellaSchedaResponse> cancellaScheda(String idAppalto,EModalitaInvio xModalitaInvio, String idScheda, String motivo,String xTipologicaScheda, Long syscon
		...
		try {			
			...
			try {
				...
				res = comunicaPostPubblicazioneResourceV1Api.v1ProtectedComunicaPostPubblicazioneCancellaSchedaPost(xSAcodiceAUSA, xTenantId, xUserCodiceFiscale, xUserIdpType, xUserLoa, xUserRole, body, xSACodiceFiscale, xModalitaInvio, xRegCodiceComponente, xRegCodicePiattaforma, xTipologicaScheda, xVersioneTracciato);
	...
	public BaseResponsePcp presaCarico(Long codGara, Long syscon, String cf, String loa, String idp) {
		...
		} catch (AppaltoPcpCastException e) {
			res = this.schedePcpManagerV102.presaCarico(codGara, syscon, cf, loa, idp);
		} catch (Exception e) {
			res.setInserito(false);		
