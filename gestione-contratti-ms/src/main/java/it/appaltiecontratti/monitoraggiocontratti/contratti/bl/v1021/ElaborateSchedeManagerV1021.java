...
@Component(value = "ElaborateSchedeManagerV1021")
public class ElaborateSchedeManagerV1021 extends AbstractManager {

    /** Logger di classe. */
    private static final Logger logger = LogManager.getLogger(ElaborateSchedeManagerV1021.class);
    ...
    //private Map<String,Map<String,List<Map<String, Object>>>> offertePresentate = new HashMap<String,Map<String,List<Map<String, Object>>>>();
        ...
        try {
            ...
            if(schedaSC1Type.getAnacForm().getDatiContratto().getCig().size() == 1) {
                ...
                if(!allineaDaPcp && countFase > 0){
                    ...
                    FaseInizialeEsecuzioneEntry faseIniz = this.contrattiMapper.getFaseInizialeEsecuzione(codGara, codLotto, 1L);
                    if(allineaDaPcp) {
        ...
        try{
            ...
            if(schedaCM1Type != null && schedaCM1Type.getAnacForm() != null){
                ...
                if(schedaCM1Type.getAnacForm().getComunicazione() != null){
                    ...
                    if(schedaCM1Type.getAnacForm().getComunicazione().getCup() != null && !schedaCM1Type.getAnacForm().getComunicazione().getCup().isEmpty() &&
                            StringUtils.isNotBlank(schedaCM1Type.getAnacForm().getComunicazione().getCup().get(0))){
                        cup = schedaCM1Type.getAnacForm().getComunicazione().getCup().get(0);
                        ...
                        for (String c : schedaCM1Type.getAnacForm().getComunicazione().getCup()){
                            if(c != null && c.length() <= 15) {
                                this.contrattiMapper.deleteW9LottoCup(codGara, codLotto);
                                this.contrattiMapper.insertW9lottcup(codGara, codLotto, 1L, c, null);
                            }
                        }
                    }
        ...
        try{
            ...
            if(schedaCM2Type != null && schedaCM2Type.getAnacForm() != null){
                ...
                if(schedaCM2Type.getAnacForm().getComunicazione() != null){
                    ...
                    if(schedaCM2Type.getAnacForm().getComunicazione().getCup() != null && !schedaCM2Type.getAnacForm().getComunicazione().getCup().isEmpty() &&
                            StringUtils.isNotBlank(schedaCM2Type.getAnacForm().getComunicazione().getCup().get(0))){
                        cup = schedaCM2Type.getAnacForm().getComunicazione().getCup().get(0);
                        if(cup != null && cup.length() <= 15) {
                            this.contrattiMapper.updateCup(codGara, codLotto, cup);
                            this.contrattiMapper.updateEsenteCupSNo(codGara, codLotto);
                        }
                        for (String c : schedaCM2Type.getAnacForm().getComunicazione().getCup()){
                            if(c != null && c.length() <= 15) {
                                this.contrattiMapper.deleteW9LottoCup(codGara, codLotto);
                                this.contrattiMapper.insertW9lottcup(codGara, codLotto, 1L, c, null);
                            }
    ...
    private void insertFaseSottoscrizioneContratto(String codein, Long syscon, String idScheda, Long codGara,
                                                   Long codLotto, FaseInizialeEsecuzioneInsertForm fase, Date dataCreazione, String statoScheda, String schedaJson, boolean allineaDaPcp, FaseInizialeEsecuzioneEntry faseIniz) throws Exception {
    ...
    public Map<String,Map<String,List<Map<String, Object>>>> elaborateAggSchedaA712Type(SchedaA712Type schedaAgg, ResponseElaborateAppaltoPcp res,
        ...
        try {
            ...
            for (AggiudicazioneA712Type agg : schedaAgg.getAnacForm().getAggiudicazioni()) {
                ...
                inserisciAggiudicazioneW9Appa(codGara, codLotto, agg.getQuadroEconomicoStandard(), null, valoreSogliaAnomalia, numeroOfferteAmmesse, dataAggiudicazione, importo, ribasso, offAumento);
    ...
    public Map<String,Map<String,List<Map<String, Object>>>> elaborateAggSchedaA711Type(SchedaA711Type schedaAgg, ResponseElaborateAppaltoPcp res,
                                           String codein, Long syscon, String idScheda, Date dataCreazione) throws Exception {
        Map<String,Map<String,List<Map<String, Object>>>> offertePresentate = new HashMap<String,Map<String,List<Map<String, Object>>>>();
        try {
    ...
    public Map<String,Map<String,List<Map<String, Object>>>> elaborateAggSchedaA34Type(SchedaA34Type schedaAgg, ResponseElaborateAppaltoPcp res, String codein,
        ...
        try {
            ...
            for (AggiudicazioneA34Type agg : schedaAgg.getAnacForm().getAggiudicazioni()) {
                ...
                schedePcpUtils.inserisciAggiudicazioneW9fasi(codGara, codLotto, idScheda);
                inserisciAggiudicazioneW9Appa(codGara, codLotto, null, agg.getQuadroEconomicoStandard(), valoreSogliaAnomalia, numeroOfferteAmmesse, dataAggiudicazione, importo, ribasso, offAumento);
                this.schedePcpUtils.inserisciW9flussi(codGara, codLotto,codein, syscon, 1L,3L, dataCreazione);
...
//							}
        ...
        try {
            ...
            for (ElencoSoggettiType agg : elencoSoggettifiltrati) {
                ...
                if(agg != null) {
                    ...
                    Map<String, List<OfferteType>> offertePresentatePerLotto = null;
                            ...
                            offertePresentatePerLotto = offertePresentatePerLottoObj.entrySet().stream()
                                    .collect(Collectors.toMap(
                                            ...
                                            entry -> entry.getValue().stream() // Otteniamo la lista di Map<String, Object>
                                                    .map(mappa -> objectMapper.convertValue(mappa, OfferteType.class)) // Convertiamo in OfferteType
                                ...
                                if (offertePresentatePerLotto != null && offertePresentatePerLotto.size() > 0 &&
                                    ...
                                    if (ruolo != null && ruolo != 4) {
                                        ...
                                        if (p.getValue().size() > 1) {
                                            schedePcpUtils.inserisciAggiudicazioneW9Aggi(codGara, codLotto, part.getIdPartecipante().toString(), codImp, tipoRaggruppamentoAgg, tipo, ruolo, importo, ribasso, offAumento, numAggi);
                                            numAggi++;
                                        } else {
                                            schedePcpUtils.inserisciAggiudicazioneW9Aggi(codGara, codLotto, part.getIdPartecipante().toString(), codImp, null, tipo, ruolo, importo, ribasso, offAumento, numAggi);
                                            numAggi++;
                                        }
                                    }
    ...
    public void elaborateAggSchedaS2RType(SchedaS2RType schedaS2RType, ResponseElaborateAppaltoPcp res, String codein,
                                         Long syscon, String idScheda, Date dataCreazione, Map<String,Map<String,List<Map<String, Object>>>> offertePresentate) throws Exception {
        try {
            Long codGara = res.getCodGara();
            Long codLotto = null;
            ...
            //rimuovo le offerte presentate che contengono lo stesso cig
            List<ElencoSoggettiType> elencoSoggettifiltrati = new ArrayList<>(
                    schedaS2RType.getAnacForm().getElencoSoggetti().stream()
                            .collect(Collectors.toMap(ElencoSoggettiType::getCig, e -> e, (existing, replacement) -> existing))
                            .values()
            );
            for (ElencoSoggettiType agg : elencoSoggettifiltrati) {
                ...
                if(agg != null) {
                    codLotto = res.getCigCodLotMap().get(agg.getCig());
                    Long id = this.contrattiMapper.getMaxIdFaseImpresa(codGara, codLotto);
                    Map<String,List<Map<String, Object>>> offertePresentatePerLottoObj = offertePresentate.get(agg.getCig());
                    Map<String, List<OfferteType>> offertePresentatePerLotto = offertePresentatePerLottoObj.entrySet().stream()
                            .collect(Collectors.toMap(
                                    Map.Entry::getKey, // Manteniamo la chiave idPartecipante
                                    entry -> entry.getValue().stream() // Otteniamo la lista di Map<String, Object>
                                            .map(mappa -> objectMapper.convertValue(mappa, OfferteType.class)) // Convertiamo in OfferteType
                        ...
                        for (InvitatoType inv : agg.getInvitatiCheNonHannoPresentatoOfferta()) {
                            if(inv != null) {
                                ...
                                if(StringUtils.isNotBlank(CodFisc)) {
                                    ...
                                    Long ruolo = inv.getRuoloOE() != null && inv.getRuoloOE().getCodice() != null ? Long.valueOf(inv.getRuoloOE().getCodice()) : null;
                                    Long tipo = inv.getTipoOE() != null && inv.getTipoOE().getCodice() != null ? Long.valueOf(inv.getTipoOE().getCodice()) : null;
            ...
            logger.error("Errore in fase di importazione fase pcp: elaborateAggSchedaS2RType");
            throw e;
        }
    ...
    public Map<String,Map<String,List<Map<String, Object>>>> elaborateAggSchedaA137Type(SchedaA137Type schedaAgg, ResponseElaborateAppaltoPcp res,
        ...
        try {
            Long codGara = res.getCodGara();
            ContractAwardNoticeType eform  = unmarshalContractAwardNoticeType(new String(org.apache.commons.codec.binary.Base64.decodeBase64(schedaAgg.getEform().getBytes()), StandardCharsets.UTF_8));
        ...
        try {
            ...
            if(partecipanti != null){
                ...
                for (Map.Entry<String, List<PartecipanteADType>> p : listaPerPartecipante.entrySet()) {
                    ...
                    for (it.appaltiecontratti.pcp.v1021.comunicaPostPubblicazione.PartecipanteADType part : p.getValue()) {
    ...
    private void inserisciAggiudicazioneW9Appa(Long codGara, Long codLotto, it.appaltiecontratti.pcp.v1021.comunicaPostPubblicazione.QuadroEconomicoType quadroEconomicoStandard, it.appaltiecontratti.pcp.v1021.comunicaPostPubblicazione.QuadroEconomicoP7Type quadroEconomicoStandard2, Double valoreSogliaAnomalia, Long numeroOfferteAmmesse, Date dataAggiudicazione, Double importo, Double ribasso, Double offAumento) {
        try {
            ...
            if(quadroEconomicoStandard != null) {
            ...
            } else if(quadroEconomicoStandard2 != null) {
                Double importoLavori = quadroEconomicoStandard2.getImpLavori() != null ? quadroEconomicoStandard2.getImpLavori() : 0D;
                Double importoServizi = quadroEconomicoStandard2.getImpServizi() != null ? quadroEconomicoStandard2.getImpServizi() : 0D;
                Double importoForniture = quadroEconomicoStandard2.getImpForniture() != null ? quadroEconomicoStandard2.getImpForniture() : 0D;
                Double importoSubtotale = importoLavori + importoServizi + importoForniture;
                aggiudicazione.setImportoLavori(importoLavori);
                aggiudicazione.setImportoServizi(importoServizi);
                aggiudicazione.setImportoForniture(importoForniture);
                ...
                Double importoSicurezza = quadroEconomicoStandard2.getImpTotaleSicurezza() != null ? quadroEconomicoStandard2.getImpTotaleSicurezza() : 0D;
                Double importoProgettazione = quadroEconomicoStandard2.getImpProgettazione() != null ? quadroEconomicoStandard2.getImpProgettazione() : 0D;
                Double importoUlterioriSommeRib = quadroEconomicoStandard2.getUlterioriSommeNoRibasso() != null ? quadroEconomicoStandard2.getUlterioriSommeNoRibasso() : 0D;
                Double importoComplAppalto = importoSubtotale + importoSicurezza + importoProgettazione + importoUlterioriSommeRib;
                aggiudicazione.setImportoSicurezza(importoSicurezza);
                aggiudicazione.setImportoProgettazione(importoProgettazione);
                aggiudicazione.setImpNonAssog(importoUlterioriSommeRib);
                ...
                Double importoDisposizione = quadroEconomicoStandard2.getSommeADisposizione() != null ? quadroEconomicoStandard2.getSommeADisposizione() : 0D;
                Double importoSommeOpzioniRinnovi = quadroEconomicoStandard2.getSommeOpzioniRinnovi() != null ? quadroEconomicoStandard2.getSommeOpzioniRinnovi() : 0D;
                Double importoSommeripetizioni = quadroEconomicoStandard2.getSommeRipetizioni() != null ? quadroEconomicoStandard2.getSommeRipetizioni() : 0D;
                aggiudicazione.setImportoSommeOpzioniRinnovi(importoSommeOpzioniRinnovi);
                aggiudicazione.setImportoSommeripetizioni(importoSommeripetizioni);
                Double importoComplIntervento = importoComplAppalto + importoSommeOpzioniRinnovi + importoSommeripetizioni + importoDisposizione;
                aggiudicazione.setImportoComplIntervento(importoComplIntervento);
    ...
    private void inserisciAggiudicazioneW9AppaCon(Long codGara, Long codLotto,
                                                  it.appaltiecontratti.pcp.v1021.comunicaPostPubblicazione.QuadroEconomicoConcessioniType quadroEconomicoConcessioni, Double valoreSogliaAnomalia, Long numeroOfferteAmmesse, Date dataAggiudicazione, Double importo, Double ribasso, Double offAumento) {
        try {
            FaseAggInsertForm aggiudicazione = new FaseAggInsertForm();
            aggiudicazione.setCodGara(codGara);
            aggiudicazione.setCodLotto(codLotto);
